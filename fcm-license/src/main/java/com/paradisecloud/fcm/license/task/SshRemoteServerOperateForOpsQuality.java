package com.paradisecloud.fcm.license.task;
 ...
 * 只用于MTR探测
 */
public abstract class SshRemoteServerOperateForOpsQuality
{
	private static final Logger LOGGER = LoggerFactory.getLogger(SshRemoteServerOperateForOpsQuality.class);
	private Session session;// JSCH session
	private boolean logined = false;// 是否登陆

	private static final SshRemoteServerOperateForOpsQuality INSTANCE = new SshRemoteServerOperateForOpsQuality()
	{

	};

	public static SshRemoteServerOperateForOpsQuality getInstance()
	{
		return INSTANCE;
	}
