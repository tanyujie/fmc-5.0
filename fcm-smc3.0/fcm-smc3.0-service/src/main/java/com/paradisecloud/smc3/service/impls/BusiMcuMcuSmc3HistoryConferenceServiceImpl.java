...
import com.paradisecloud.smc3.task.EndConferenceTaskSmc3;
import com.paradisecloud.smc3.task.Smc3DelayTaskService;
import com.paradisecloud.smc3.task.StatisticalConferenceTaskSmc3;
...
@Transactional
@Service
public class BusiMcuMcuSmc3HistoryConferenceServiceImpl implements IBusiMcuSmc3HistoryConferenceService {
    
    private Logger logger = LoggerFactory.getLogger(BusiMcuMcuSmc3HistoryConferenceServiceImpl.class);
    ...
    @Override
    public BusiHistoryConference saveHistory(CoSpace cosapce, Call call, Smc3ConferenceContext conferenceContext)
    {
        try
        {
    ...
    @Override
    public BusiHistoryConference saveHistory(BusiHistoryConference busiHistoryConference, Smc3ConferenceContext conferenceContext)
    {
        try
        {
            ...
            if (busiHistoryConference.getConferenceEndTime() != null) {
                // 结束会议
                EndConferenceTaskSmc3 endConferenceTask = new EndConferenceTaskSmc3(busiHistoryConference.getId().toString(), 1000, busiHistoryConference.getId());
                smc3DelayTaskService.addTask(endConferenceTask);
            }

            // 会议统计
            StatisticalConferenceTaskSmc3 statisticalConferenceTask = new StatisticalConferenceTaskSmc3(busiHistoryConference.getDeptId().toString(), 10000, busiHistoryConference.getDeptId());
            smc3DelayTaskService.addTask(statisticalConferenceTask);
    ...
    @Override
    public BusiHistoryConference saveHistory(Smc3ConferenceContext conferenceContext) {
        ...
        busiHistoryConference.setConferenceStartTime(conferenceContext.getStartTime());
        busiHistoryConference.setCreateUserId(conferenceContext.getCreateUserId());
        busiHistoryConference.setMcuType(McuType.SMC3.getCode());
        busiHistoryConference.setCallId(conferenceContext.getSmc3conferenceId());
        int type = 0;
        if (conferenceContext.getAppointmentType() != null) {
            if (conferenceContext.getAppointmentType() == 2) {
                type = 2;
        ...
        StatisticalConferenceTaskSmc3 statisticalConferenceTask=new StatisticalConferenceTaskSmc3(busiHistoryConference.getDeptId().toString(),1000,busiHistoryConference.getDeptId());
        smc3DelayTaskService.addTask(statisticalConferenceTask);
        return busiHistoryConference;
    }
     ...
     */
    @Override
    public void updateBusiHistoryParticipant(Smc3ConferenceContext conferenceContext, AttendeeSmc3 attendee, boolean updateMediaInfo) {
            ...
            if (busiHistoryConference != null) {
                ...
                if (busiHistoryParticipant != null) {
                    if (busiHistoryParticipant.getJoined() == null || !busiHistoryParticipant.getJoined()) {
                        ...
                        if (updateMediaInfo) {
                            JSONObject jsonObject = Smc3WebSocketProcessor.getRealTimeMap().get(attendee.getParticipantUuid());
                            JSONObject mediainfo = Smc3RealTimeInfoProcessorMessage.toDetail(jsonObject, attendee.getSmcParticipant().getGeneralParam().getType(), attendee.getRemoteParty());
                            if(mediainfo!=null){
