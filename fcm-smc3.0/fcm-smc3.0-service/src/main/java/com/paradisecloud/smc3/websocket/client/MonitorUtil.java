...
public class MonitorUtil {
    ...
    private static Map<String, SMC3WebsocketClient> monitorWebSocketMap=new ConcurrentHashMap<>();
    ...
    public void start(int monitorInterval, SMC3WebsocketClient smc3WebsocketClient) {
        monitorInterval = monitorInterval;

        monitorWebSocketMap.put(monitorName, smc3WebsocketClient);
    ...
    private Runnable monitorAliveRunnable(){
        return ()->{
            for (Map.Entry<String, SMC3WebsocketClient> entry : monitorWebSocketMap.entrySet()) {
                try {
