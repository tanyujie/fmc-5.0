 ...
 * @version V1.0
 */
package com.paradisecloud.smc3.busi.enumer;
...
public enum AttendeeImportance
{
    ...
    {
        @Override
        public void processAttendeeWebsocketMessage(AttendeeSmc3 attendee)
        {
    ...
    MASTER(7500000, 7500000, "主会场")
    {
        ...
        public void processAttendeeWebsocketMessage(AttendeeSmc3 attendee)
        {
            ...
            attendee.setBroadcastStatus(BroadcastStatus.NO.getValue());
            attendee.setRoundRobinStatus(AttendeeRoundRobinStatus.NO.getValue());
            Smc3ConferenceContext mainConferenceContext = Smc3ConferenceContextCache.getInstance().get(attendee.getContextKey());
            if (mainConferenceContext != null && mainConferenceContext.getMasterAttendee() != attendee)
            {
                AttendeeSmc3 oldMasterAttendee = mainConferenceContext.getMasterAttendee();
                ...
                Smc3WebSocketMessagePusher.getInstance().pushConferenceMessageToAll(mainConferenceContext, WebsocketMessageType.MASTER_CHANGED, data);

                StringBuilder messageTip = new StringBuilder();
                messageTip.append("主会场已切换至【").append(attendee.getName()).append("】");
                Smc3WebSocketMessagePusher.getInstance().pushConferenceMessageToAll(mainConferenceContext, WebsocketMessageType.MESSAGE_TIP, messageTip);
        ...
        public void processAttendeeWebsocketMessage(AttendeeSmc3 attendee)
        {
            ...
            if (attendee.getUpdateMap().size() > 1)
            {
                ...
                if (mainConferenceContext != null)
                {
                    ...
                    Smc3WebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(mainConferenceContext, WebsocketMessageType.MESSAGE_TIP, messageTip);
                    Smc3WebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(mainConferenceContext, WebsocketMessageType.ATTENDEE_UPDATE, attendee.getUpdateMap());
                }
            }
        }
    },
     ...
     * @param attendee void
     */
    public abstract void processAttendeeWebsocketMessage(AttendeeSmc3 attendee);
