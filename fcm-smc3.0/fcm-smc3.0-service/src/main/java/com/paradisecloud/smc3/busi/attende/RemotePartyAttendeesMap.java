 ...
 * @version V1.0  
 */
public class RemotePartyAttendeesMap extends ConcurrentHashMap<String, Map<String, AttendeeSmc3>>
{
    ...
    public void addAttendee(AttendeeSmc3 attendee)
    {
        if(attendee==null|| Strings.isBlank(attendee.getRemoteParty())){
            return;
        }
        Map<String, AttendeeSmc3> uuidAttendeeMap = get(attendee.getRemoteParty());
    ...
    public AttendeeSmc3 removeAttendeeByRemotePartyAndUuid(String uri, String uuid)
    {
        Map<String, AttendeeSmc3> m = get(uri);
        if (m != null)
        {
            AttendeeSmc3 a = m.remove(uuid);
            if (m.isEmpty())
            {
                remove(uri);
            }
