 ...
 * @version V1.0  
 */
public class RemotePartyAttendeesMap extends ConcurrentHashMap<String, Map<String, AttendeeHwcloud>>
{
    ...
    public void addAttendee(AttendeeHwcloud attendee)
    {
        if(attendee==null||attendee.getRemoteParty()==null){
            return;
        }
        Map<String, AttendeeHwcloud> uuidAttendeeMap = get(attendee.getRemoteParty());
    ...
    public AttendeeHwcloud removeAttendeeByRemotePartyAndUuid(String uri, String uuid)
    {
        if(uri==null){
            return null;
        }
        Map<String, AttendeeHwcloud> m = get(uri);
        if (m != null)
        {
            AttendeeHwcloud a = m.remove(uuid);
            if (m.isEmpty())
            {
                remove(uri);
            }
