...
public enum AttendeeImportance {
    ...
    COMMON(null, null, "分会场") {
        @Override
        public void processAttendeeWebsocketMessage(AttendeeHwcloud attendee) {
    ...
    MASTER(7500000, 7500000, "主会场") {
        ...
        public void processAttendeeWebsocketMessage(AttendeeHwcloud attendee) {
            ...
            attendee.setBroadcastStatus(BroadcastStatus.NO.getValue());
            attendee.setRoundRobinStatus(AttendeeRoundRobinStatus.NO.getValue());
            HwcloudConferenceContext mainConferenceContext = HwcloudConferenceContextCache.getInstance().get(attendee.getContextKey());
            if (mainConferenceContext != null && mainConferenceContext.getMasterAttendee() != attendee) {
                AttendeeHwcloud oldMasterAttendee = mainConferenceContext.getMasterAttendee();
                ...
                HwcloudWebSocketMessagePusher.getInstance().pushConferenceMessageToAll(mainConferenceContext, WebsocketMessageType.MASTER_CHANGED, data);

                StringBuilder messageTip = new StringBuilder();
                messageTip.append("主会场已切换至【").append(attendee.getName()).append("】");
                HwcloudWebSocketMessagePusher.getInstance().pushConferenceMessageToAll(mainConferenceContext, WebsocketMessageType.MESSAGE_TIP, messageTip);
        ...
        public void processAttendeeWebsocketMessage(AttendeeHwcloud attendee) {
            ...
            if (attendee.getUpdateMap().size() > 1) {
                ...
                if (mainConferenceContext != null) {
                    ...
                    HwcloudWebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(mainConferenceContext, WebsocketMessageType.MESSAGE_TIP, messageTip);
                    HwcloudWebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(mainConferenceContext, WebsocketMessageType.ATTENDEE_UPDATE, attendee.getUpdateMap());
                }
            }
        }
    },
     ...
     */
    public abstract void processAttendeeWebsocketMessage(AttendeeHwcloud attendee);
