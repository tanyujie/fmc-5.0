...
import com.paradisecloud.fcm.huaweicloud.huaweicloud.task.EndConferenceTaskHwcloud;
import com.paradisecloud.fcm.huaweicloud.huaweicloud.task.HwcloudDelayTaskService;
import com.paradisecloud.fcm.huaweicloud.huaweicloud.task.StatisticalConferenceTaskHwcloud;
...
@Transactional
@Service
public class BusiMcuHwcloudHistoryConferenceServiceImpl implements IBusiMcuHwcloudHistoryConferenceService {

    private Logger logger = LoggerFactory.getLogger(BusiMcuHwcloudHistoryConferenceServiceImpl.class);
    ...
    @Override
    public BusiHistoryConference saveHistory(CoSpace cosapce, Call call, HwcloudConferenceContext conferenceContext)
    {
        try
        {
    ...
    @Override
    public BusiHistoryConference saveHistory(BusiHistoryConference busiHistoryConference, HwcloudConferenceContext conferenceContext)
    {
        try
        {
            ...
            if (busiHistoryConference.getConferenceEndTime() != null) {
                // 结束会议
                EndConferenceTaskHwcloud endConferenceTask = new EndConferenceTaskHwcloud(busiHistoryConference.getId().toString(), 1000, busiHistoryConference.getId());
                hwcloudDelayTaskService.addTask(endConferenceTask);
            }

            // 会议统计
            StatisticalConferenceTaskHwcloud statisticalConferenceTask = new StatisticalConferenceTaskHwcloud(busiHistoryConference.getDeptId().toString(), 10000, busiHistoryConference.getDeptId());
            hwcloudDelayTaskService.addTask(statisticalConferenceTask);
    ...
    @Override
    public BusiHistoryConference saveHistory(HwcloudConferenceContext conferenceContext) {
        ...
        StatisticalConferenceTaskHwcloud statisticalConferenceTask=new StatisticalConferenceTaskHwcloud(busiHistoryConference.getDeptId().toString(),1000,busiHistoryConference.getDeptId());
        hwcloudDelayTaskService.addTask(statisticalConferenceTask);
        return busiHistoryConference;
    }
     ...
     */
    @Override
    public void updateBusiHistoryParticipant(HwcloudConferenceContext conferenceContext, AttendeeHwcloud attendee, boolean updateMediaInfo) {
            ...
            if (busiHistoryConference != null) {
                ...
                if (busiHistoryParticipant != null) {
                    if (busiHistoryParticipant.getJoined() == null || !busiHistoryParticipant.getJoined()) {
                        ...
                        if (updateMediaInfo) {
                            NetConditionNotifyParticipant netConditionNotifyParticipant = NetConditionNotifyParticipantCache.getInstance().get(attendee.getParticipantUuid());
                            JSONObject mediainfo = NetConditionNotifyParticipantCache.toDetail(netConditionNotifyParticipant);
                            if(mediainfo!=null){
                                busiHistoryParticipant.setMediaInfo(mediainfo.toJavaObject(Map.class));
                            }
                            busiHistoryParticipantMapper.updateBusiHistoryParticipant(busiHistoryParticipant);
                        }
                    }
    ...
    public List<BusiHistoryConference> selectBusiHistoryConferenceListNotEnd() {
        return busiHistoryConferenceMapper.selectNotEndHistoryConferenceList(McuType.MCU_HWCLOUD.getCode());
    }
}
