 ...
 */
public enum HwcloudWebsocketMessageType {
    /**
     * 会监号码
     */
     ...
     */
    private String name;
    private static final Map<String, HwcloudWebsocketMessageType> MAP = new HashMap<>();
    static
    {
        for (HwcloudWebsocketMessageType recordType : values())
        {
            MAP.put(recordType.value, recordType);
        }
    }

    HwcloudWebsocketMessageType(String value, String name)
    {
        this.value = value;
        this.name = name;
    }
    ...
    public HwcloudWebsocketMessage create(Object data)
    {
        return new HwcloudWebsocketMessage(value, data);
    }


    public HwcloudWebsocketMessage create(Object data, String destination)
    {
        return new HwcloudWebsocketMessage(value, data, destination);
    }

    public static HwcloudWebsocketMessageType convert(String value)
    {
        HwcloudWebsocketMessageType t = MAP.get(value);
        if (t == null)
        {
            throw new SystemException("非法的" + HwcloudWebsocketMessageType.class.getSimpleName() + "枚举值：" + value);
        }
        return t;
    }
}
