...
public class RegisteredAttendeeUpdateProcessor extends AttendeeUpdateProcessor {
     ...
     */
    public RegisteredAttendeeUpdateProcessor(SmcParitipantsStateRep.ContentDTO participant, AttendeeHwcloud a, HwcloudConferenceContext conferenceContext) {
    ...
    public void process() {
        ...
        // FME类型的与会者
        else if (attendee instanceof McuAttendeeHwcloud) {
            processFmeAttendee();
        }
        else if (attendee instanceof CropDirAttendeeHwcloud) {
            processCropAttendeeUpdate();
        }
    }
    ...
    private void processFmeAttendee() {
        if (attendee instanceof McuAttendeeHwcloud) {
            processMcuAttendee();
        }
    }
    ...
    private void processMcuAttendee() {
        McuAttendeeHwcloud mcuAttendee = (McuAttendeeHwcloud) attendee;
        ...
        if (baseConferenceContext == null) {
            ...
            if (viewTemplateConferenceList == null || viewTemplateConferenceList.size() == 0) {
                ...
                HwcloudWebSocketMessagePusher.getInstance().upwardPushConferenceMessage(conferenceContext, WebsocketMessageType.ATTENDEE_DELETE, updateMap);
                return;
            }
        }
    ...
    private void processCropAttendeeUpdate() {
        // 消息推送
        AttendeeMessagePusher.getInstance().pushMixingStatusMessage(attendee, conferenceContext);
        AttendeeMessagePusher.getInstance().pushMeetingJoinedMessage(attendee, participant, conferenceContext);
        AttendeeMessagePusher.getInstance().pushOnlineMessage(attendee, conferenceContext);
