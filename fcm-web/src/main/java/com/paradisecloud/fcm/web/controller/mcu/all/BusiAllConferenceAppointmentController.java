 ...
 */
@RestController
@RequestMapping("/busi/mcu/all/conferenceAppointment")
@Tag(name = "会议预约记录")
public class BusiAllConferenceAppointmentController extends BaseController {
    @Resource
    ...
    public RestResponse list(@RequestBody ViewConferenceAppointment viewConferenceAppointment) {
        ...
        boolean initiFlag = FmeDataCache.initiFlag;
        if (!initiFlag) {
            return RestResponse.success(new PaginationData<>());
        }
        startPage();
        ...
        for (ViewConferenceAppointment viewConferenceAppointmentTemp : list) {
            ...
            try {
                BusiTemplateConference busiTemplateConference = busiTemplateConferenceMapper.selectBusiTemplateConferenceById(viewConferenceAppointmentTemp.getTemplateId());
                if (busiTemplateConference.getUpCascadeId() != null) {
                    viewConferenceAppointmentVo.setDownCascade(true);
                } else {
                    viewConferenceAppointmentVo.setDownCascade(true);
                }
                List<ModelBean> downCascades = new ArrayList<>();
                {
                    ...
                    viewTemplateConferenceCon.setUpCascadeId(viewConferenceAppointmentTemp.getTemplateId());
                    viewTemplateConferenceCon.setUpCascadeMcuType(viewConferenceAppointmentTemp.getMcuType());
...
//                }
                if (downCascades.size() > 0) {
                    viewConferenceAppointmentVo.setUpCascade(true);
                } else {
                    viewConferenceAppointmentVo.setUpCascade(true);
                }
                viewConferenceAppointmentVo.setDownCascades(downCascades);
            } catch (Exception e) {
            }
     ...
     * 从场景历史开启快速会议
     */
    @PostMapping(value = "/addFromModeHistory")
    @Operation(summary = "从场景历史开启快速会议", description = "从场景历史开启快速会议")
    public RestResponse addFromModeHistory(@RequestBody JSONObject jsonObject) {
        Long hisConferenceId = jsonObject.getLong("hisConferenceId");
        BusiHistoryConference busiHistoryConference = busiHistoryConferenceService.selectBusiHistoryConferenceById(hisConferenceId);
        if (busiHistoryConference != null) {
            if (busiHistoryConference.getTemplateId() != null) {
                ViewTemplateConference viewTemplateConference = viewTemplateConferenceMapper.selectViewTemplateConferenceById(busiHistoryConference.getMcuType(), busiHistoryConference.getTemplateId());
                BaseConferenceContext baseConferenceContext = AllConferenceContextCache.getInstance().get(viewTemplateConference.getConferenceId());
                if (baseConferenceContext != null) {
                    return RestResponse.fail("该场景正在会议！");
                }
                if (viewTemplateConference.getConferenceMode() != null) {
                    JSONObject appointmentParams = createAppointmentParams(viewTemplateConference, busiHistoryConference);
                    return add(appointmentParams);
                } else {
                    return RestResponse.fail("非场景历史无法开启会议！");
                }
            }
        } else {
            RestResponse.fail("历史会议不存在！");
        }
        return RestResponse.fail();
    }

    private JSONObject createAppointmentParams(ViewTemplateConference viewTemplateConference, BusiHistoryConference busiHistoryConference) {
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("deptId", viewTemplateConference.getDeptId());
        jsonObject.put("isAutoCreateTemplate", 2);
        jsonObject.put("repeatRate", 1);
        jsonObject.put("status", 1);
        jsonObject.put("type", 2);
        jsonObject.put("mcuType", viewTemplateConference.getMcuType());
        jsonObject.put("templateId", viewTemplateConference.getId());

        JSONObject params = new JSONObject();
        params.put("type", 2);
        params.put("deptId", viewTemplateConference.getDeptId());
        params.put("duration", viewTemplateConference.getDurationTime() / 60);
        params.put("conferenceNumber", busiHistoryConference.getNumber());
     ...
     * 删除场景历史记录
     */
    @DeleteMapping("/deleteModeHistory/{hisConferenceIds}")
    @Operation(summary = "删除场景历史记录", description = "删除场景历史记录")
    public RestResponse deleteModeHistory(@PathVariable("hisConferenceIds") Long[] hisConferenceIds) {
        for (Long hisConferenceId : hisConferenceIds) {
            BusiHistoryConference busiHistoryConference = busiHistoryConferenceService.selectBusiHistoryConferenceById(hisConferenceId);
            if (busiHistoryConference != null) {
                if (busiHistoryConference.getTemplateId() != null) {
//                BusiHistoryConference busiHistoryConferenceUpdate = new BusiHistoryConference();
//                busiHistoryConferenceUpdate.setId(busiHistoryConference.getId());
//                busiHistoryConferenceUpdate.setTemplateId(null);
//                int rows = busiHistoryConferenceMapper.updateModeHistoryConference(busiHistoryConferenceUpdate);
//                return toAjax(rows);
                    BusiHistoryConference busiHistoryConferenceDelete = new BusiHistoryConference();
                    busiHistoryConferenceDelete.setTemplateId(busiHistoryConference.getTemplateId());
                    busiHistoryConferenceMapper.deleteModeHistoryConference(busiHistoryConferenceDelete);
                }
            } else {
                if (hisConferenceIds != null && hisConferenceIds.length == 1) {
                    RestResponse.fail("历史会议不存在！");
                }
            }
        }
     ...
     * 删除场景历史记录
     */
    @DeleteMapping("/deleteAllModeHistory")
    @Operation(summary = "删除所有场景历史记录", description = "删除所有场景历史记录")
    public RestResponse deleteModeHistory() {
        busiHistoryConferenceMapper.deleteAllModeHistoryConference();
        return RestResponse.success();
    }
     ...
     * @return
     */
    @GetMapping(value = "/getIdleConferenceNumber")
    public RestResponse getIdleConferenceNumber(Long deptId, String mcuType) {
        Long conferenceNumber = null;
        Long startConferenceNumber = null;
        Long endConferenceNumber = null;
        BusiConferenceNumberSection busiConferenceNumberSectionCon = new BusiConferenceNumberSection();
        busiConferenceNumberSectionCon.setMcuType(mcuType);
        busiConferenceNumberSectionCon.setDeptId(deptId);
        busiConferenceNumberSectionCon.setSectionType(ConferenceNumberSectionType.COMMON.getValue());
        List<BusiConferenceNumberSection> busiConferenceNumberSectionList = busiConferenceNumberSectionMapper.selectBusiConferenceNumberSectionList(busiConferenceNumberSectionCon);
        if (busiConferenceNumberSectionList.size() == 0) {
            return RestResponse.fail("该租户还未绑定会议号段，请联系系统管理员进行绑定！");
        }
        for (BusiConferenceNumberSection busiConferenceNumberSection : busiConferenceNumberSectionList) {
            startConferenceNumber = busiConferenceNumberSection.getStartValue();
            endConferenceNumber = busiConferenceNumberSection.getEndValue();
            conferenceNumber = startConferenceNumber;
        }
        BusiConferenceNumber busiConferenceNumber = busiConferenceNumberService.autoCreateConferenceNumber(deptId, mcuType, false, true);
        if (busiConferenceNumber != null) {
            Integer conferenceLimit = LicenseCache.getInstance().getConferenceLimit();
            if (conferenceLimit != null) {
                if (busiConferenceNumber.getId() < conferenceNumber + conferenceLimit) {
                    conferenceNumber = busiConferenceNumber.getId();
                } else {
                    for (long i = startConferenceNumber; i < startConferenceNumber + conferenceLimit && i <= endConferenceNumber; i++) {
                        BusiConferenceNumber busiConferenceNumberExist = busiConferenceNumberService.selectBusiConferenceNumberById(i);
                        if (busiConferenceNumberExist == null || busiConferenceNumberExist.getStatus() == ConferenceNumberStatus.IDLE.getValue()) {
                            conferenceNumber = i;
                            break;
                        }
                    }
        ...
        return RestResponse.success(conferenceNumber);
    }
    ...
    public RestResponse add(@RequestBody JSONObject jsonObject) {
        ...
        try {
            if (busiConferenceAppointment.getEndTime() == null) {
                ...
                if (durationObj != null) {
                    ...
                    if (busiConferenceAppointment.getType() == 2) {
                        ...
                        startTimeStr = DateUtil.convertDateToString(startTime, "yyyy-MM-dd HH:mm:ss");
                        busiConferenceAppointment.setStartTime(startTimeStr);
                    }
                    Timestamp endTime = new Timestamp(Timestamp.valueOf(startTimeStr).getTime() + (duration * 60 * 60 * 1000));
                    busiConferenceAppointment.setEndTime(DateUtil.convertDateToString(endTime, "yyyy-MM-dd HH:mm:ss"));
                }
            }
            if (type == null) {
                type = 1;
        ...
        if (durationObj == null) {
            String startTimeStr = busiConferenceAppointment.getStartTime();
            String endTimeStr = busiConferenceAppointment.getEndTime();
            long duration = (Timestamp.valueOf(endTimeStr).getTime() - Timestamp.valueOf(startTimeStr).getTime()) / (1000 * 60 * 60);
            busiConferenceAppointment.getParams().put("duration", (int) duration);
        }
        Map<String, Object> resultMap = null;
        String region = ExternalConfigCache.getInstance().getRegion();
        switch (mcuType) {
            case FME: {
                if (type == 2 && !Objects.equals("ops", region)){
                ...
                if (busiConferenceAppointment != null && busiConferenceAppointment.getIsAutoCreateTemplate() == 1) {
                    ...
                    try {
                        ...
                        String conferenceName = (String) busiConferenceAppointment.getParams().get("conferenceName");
                        resultMap.put("conferenceName", conferenceName);
                    } catch (Exception e) {
                        try {
                    ...
                    //云会议本地数据
                    try {
                        Map<String, Object> businessProperties = null;
                        if (busiConferenceAppointment.getParams().get("businessProperties") != null) {
                            businessProperties = (Map<String, Object>) busiConferenceAppointment.getParams().get("businessProperties");
                        }
                        if (businessProperties != null) {
                            Object cloudMcuType = businessProperties.get("cloudMcuType");
                            if (cloudMcuType != null) {
                                List<McuType> cloudMcuTypeList = new ArrayList<>();
                                if (cloudMcuType instanceof ArrayList) {
                                    ArrayList jsonArray = (ArrayList) cloudMcuType;
                                    for (Object cloudMcuTypeObj : jsonArray) {
                                        String cloudMcuTypeStr = (String) cloudMcuTypeObj;
                                        if (McuType.MCU_TENCENT.getCode().equals(cloudMcuTypeStr)) {
                                            cloudMcuTypeList.add(McuType.MCU_TENCENT);
                                        }
                                        if(McuType.SMC3.getCode().equals(cloudMcuTypeStr)){
                                            cloudMcuTypeList.add(McuType.SMC3);
                                        }
                                        if(McuType.SMC2.getCode().equals(cloudMcuTypeStr)){
                                            cloudMcuTypeList.add(McuType.SMC2);
                                        }
                                    }
                                }
                                if (cloudMcuType instanceof String) {
                                    String cloudMcuTypeStr = (String) cloudMcuType;
                                    if (McuType.MCU_TENCENT.getCode().equals(cloudMcuTypeStr)) {
                                        cloudMcuTypeList.add(McuType.MCU_TENCENT);
                                    }
                                    if (McuType.SMC3.getCode().equals(cloudMcuTypeStr)) {
                                ...
                                for (int m = 0; m < cloudMcuTypeList.size(); m++) {
                                    McuType cloudMcuTypeTemp = cloudMcuTypeList.get(m);
                                    if (cloudMcuTypeTemp == McuType.MCU_TENCENT) {
                                        BusiMcuTencentTemplateConference busiMcuTencentTemplateConference = new BusiMcuTencentTemplateConference();
                                        busiMcuTencentTemplateConference.setUpCascadeId(busiConferenceAppointment.getTemplateId());
                                        busiMcuTencentTemplateConference.setUpCascadeMcuType(mcuType.getCode());
                                    ...
                                    if (cloudMcuTypeTemp == McuType.SMC3) {

                                        Smc3Bridge bridgesByDept = Smc3BridgeCache.getInstance().getBridgesByDept(busiConferenceAppointment.getDeptId());
                                        ...
                                        busiMcuSmc3TemplateConference.setUpCascadeId(busiConferenceAppointment.getTemplateId());
                                        busiMcuSmc3TemplateConference.setUpCascadeMcuType(mcuType.getCode());
                        ...
                        switch (mcuType) {
                            case FME: {
                                ...
                                if (contextKey != null) {
                                    ...
                                    if(YesOrNo.convert(busiConferenceAppointment.getIsStart()) == YesOrNo.NO){
                                        busiConferenceAppointment.setStatus(AppointmentConferenceStatus.ENABLED.getValue());
                                        busiConferenceAppointment.setIsStart(YesOrNo.NO.getValue());
                                        busiConferenceAppointmentService.updateBusiConferenceAppointment(busiConferenceAppointment, false);
                                    }
                                }else {
                                    busiConferenceAppointment.setStatus(AppointmentConferenceStatus.ENABLED.getValue());
                ...
                case FME: {
                    try {
                        if (type > 0) {
                            ...
                            LoginUser loginUserA = SecurityUtils.getLoginUser();
                            BusiUser busiUser = busiUserService.selectUserById(loginUserA.getUser().getUserId());
                            String phonenumber = busiUser.getPhonenumber();
                            String cfname = resultMap.get("conferenceName") + "";
                            String cfNumber = resultMap.get("conferenceNumber") + "";
                            ...
                            if (businessProperties != null) {
                                Object cloudMcuType = businessProperties.get("cloudMcuType");
                                if (cloudMcuType != null) {
                                    if (Objects.equals(cloudMcuType, McuType.MCU_TENCENT.getCode())) {
                                        if (Strings.isNotBlank(phonenumber)) {
                                            ...
                                            if (Objects.equals("ops", region)) {
                                                CloudSmsLocaltoRemoteTask cloudSmsLocaltoRemoteTask = new CloudSmsLocaltoRemoteTask(appointmentId + "TX", 1000, cfname, cfNumber, phonenumber, startTime, null, NotifyType.ADMIN_TENCENT_MEETING_START);
                                                taskService.addTask(cloudSmsLocaltoRemoteTask);
                                            } else {
                                                OpsNotifyTask opsNotifyTask = new OpsNotifyTask("sendSms" + appointmentId, 100, cfname, cfNumber, phonenumber, startTime, null, NotifyType.ADMIN_TENCENT_MEETING_START.name());
                                        ...
                                        if (Strings.isNotBlank(phonenumber)) {
                                            ...
                                            if (Objects.equals("ops", region)) {
                                                CloudSmsLocaltoRemoteTask cloudSmsLocaltoRemoteTask = new CloudSmsLocaltoRemoteTask(appointmentId + "BK", 1000, cfname, cfNumber, phonenumber, startTime, null, NotifyType.ADMIN_MEETING_BOOK);
                                                taskService.addTask(cloudSmsLocaltoRemoteTask);
                                            } else {
                                                OpsNotifyTask opsNotifyTask = new OpsNotifyTask("sendSms" + appointmentId, 100, cfname, cfNumber, phonenumber, startTime, null, NotifyType.ADMIN_MEETING_BOOK.name());
                                                taskService.addTask(opsNotifyTask);
                                            }
                                        }
                                    }
                        ...
                        logger.info("预约会议SMS发送 error 1 " + e.getMessage());
                    }
                    break;
                }
        ...
        switch (mcuType) {
            case FME: {
                ...
                if (i > 0) {
                    ...
                    BusiConferenceAppointment conferenceAppointment = busiConferenceAppointmentService.selectBusiConferenceAppointmentById(busiConferenceAppointment.getId());
                    Long templateId = conferenceAppointment.getTemplateId();
                    BusiTemplateConference tc = busiTemplateConferenceMapper.selectBusiTemplateConferenceById(templateId);
                    Long appointmentId = conferenceAppointment.getId();

                    String region = ExternalConfigCache.getInstance().getRegion();
                    String cfname = tc.getName();
                    String cfNumber = tc.getConferenceNumber() + "";
                    String startTime = busiConferenceAppointment.getStartTime();
                        ...
                        if (cloudMcuType != null) {
                            if (Objects.equals(cloudMcuType, McuType.MCU_TENCENT.getCode())) {
                                if (Strings.isNotBlank(phonenumber)) {
                                    ...
                                    if (Objects.equals("ops", region)) {
                                        CloudSmsLocaltoRemoteTask cloudSmsLocaltoRemoteTask = new CloudSmsLocaltoRemoteTask(appointmentId + "TXE", 1000, cfname, cfNumber, phonenumber, startTime, null, NotifyType.ADMIN_TENCENT_MEETING_START);
                                        taskService.addTask(cloudSmsLocaltoRemoteTask);
                                    } else {
                                        OpsNotifyTask opsNotifyTask = new OpsNotifyTask("sendSms_e" + appointmentId, 100, cfname, cfNumber, phonenumber, startTime, null, NotifyType.ADMIN_TENCENT_MEETING_START.name());
                                        taskService.addTask(opsNotifyTask);
                                    }
                                }
                            } else {
                                if (Strings.isNotBlank(phonenumber)) {
                                    if (Objects.equals("ops", region)) {
                                        CloudSmsLocaltoRemoteTask cloudSmsLocaltoRemoteTask = new CloudSmsLocaltoRemoteTask(appointmentId + "TXE", 1000, cfname, cfNumber, phonenumber, startTime, null, NotifyType.ADMIN_MEETING_BOOK);
                                        taskService.addTask(cloudSmsLocaltoRemoteTask);
                                    } else {
                                        OpsNotifyTask opsNotifyTask = new OpsNotifyTask("sendSms_e" + appointmentId, 100, cfname, cfNumber, phonenumber, startTime, null, NotifyType.ADMIN_MEETING_BOOK.name());
                                        taskService.addTask(opsNotifyTask);
                                    }
                ...
                if (i > 0) {
                    ...
                    try {
                        if (viewTemplateConference != null) {
                            String region = ExternalConfigCache.getInstance().getRegion();
                            logger.info("ops region" + region);
                            if (Objects.equals("ops", region)) {
                                String cfname = viewTemplateConference.getName();
                                String cfNumber = viewTemplateConference.getConferenceNumber() + "";
                                String startTime = viewConferenceAppointment.getStartTime();
                                String endTime = viewConferenceAppointment.getEndTime();
                                ...
                                if (admin != null && admin.getPhonenumber() != null) {
                                    CloudSmsLocaltoRemoteTask cloudSmsLocaltoRemoteTask = new CloudSmsLocaltoRemoteTask(viewConferenceAppointment.getId().toString(), 1000, cfname, cfNumber, admin.getPhonenumber(), startTime, endTime, NotifyType.ADMIN_MEETING_CANCEL);
                                    taskService.addTask(cloudSmsLocaltoRemoteTask);
                                }
                            }
                        }
                    } catch (Exception e) {
                        logger.info("smscloudtencent error 3 " + e.getMessage());
                    }
                }
                return toAjax(i);
            }
