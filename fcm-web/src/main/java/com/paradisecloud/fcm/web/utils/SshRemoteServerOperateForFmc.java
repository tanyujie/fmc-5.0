...
public class SshRemoteServerOperateForFmc
{
	private static final Logger LOGGER = LoggerFactory.getLogger(SshRemoteServerOperateForFmc.class);
	...
	public void sshRemoteCallLogin(String ipAddress, String userName, String password, Integer port) throws Exception 
	{
		...
		try 
		{
			...
			//通过Session建立连接
			 session.setConfig(SshConfigConstant.STRICT_HOST_KEY_CHECKING, SshConfigConstant.NO);
			 session.connect();
			 logined = true;
		} 
...
//			channelSftp.exit();
	...
	public static Boolean pingIpAndPort(String ip, Integer port) {
