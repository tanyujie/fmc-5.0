package com.paradisecloud.fcm.web.controller.mobile.report;
...
public class MobileHistoryController extends BaseController {
    private static final Logger log = LoggerFactory.getLogger(MobileHistoryController.class);
    @Autowired
    ...
    public RestResponse getDeptRecordCounts()
    {
        ...
        List<DeptRecordCount> deptRecordCounts = iBusiHistoryConferenceService.getDeptRecordCounts();
        if(CollectionUtils.isNotEmpty(deptRecordCounts)){
            deptRecordCounts = deptRecordCounts.stream().filter(s -> Objects.equals(s.getDeptId(), deptId)).collect(Collectors.toList());
        }
        return RestResponse.success(deptRecordCounts);
    }
    ...
    @Operation(description = "下载会议纪要文件DOC")
    public void downMeetingMinutesDocFile(HttpServletRequest request, HttpServletResponse response, @PathVariable Long hisConferenceId) {
        BusiHistoryConference busiHistoryConference = iBusiHistoryConferenceService.selectBusiHistoryConferenceById(hisConferenceId);
        Assert.notNull(busiHistoryConference, "该历史会议不存在！");
        String minutesFilePath = MinutesFileHandler.generateFileDocPath(busiHistoryConference.getCoSpace(), busiHistoryConference.getNumber(), busiHistoryConference.getId());
        File downloadFile = new File(minutesFilePath);
        if (!downloadFile.exists()) {
            Assert.isTrue(false, "该历史会议纪要DOC不存在！");
        }
