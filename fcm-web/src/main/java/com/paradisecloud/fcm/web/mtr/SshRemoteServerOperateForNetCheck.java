 ...
 * 只用于MTR探测
 */
public abstract class SshRemoteServerOperateForNetCheck
{
	private static final Logger LOGGER = LoggerFactory.getLogger(SshRemoteServerOperateForNetCheck.class);
	private Session session;// JSCH session
	private boolean logined = false;// 是否登陆

	private static final SshRemoteServerOperateForNetCheck INSTANCE = new SshRemoteServerOperateForNetCheck()
	{

	};

	public static SshRemoteServerOperateForNetCheck getInstance()
	{
		return INSTANCE;
	}
