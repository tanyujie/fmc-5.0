 ...
 * 只用于MTR探测
 */
public abstract class SshRemoteServerOperateForMtr
{
	private static final Logger LOGGER = LoggerFactory.getLogger(SshRemoteServerOperateForMtr.class);
	private Session session;// JSCH session
	private boolean logined = false;// 是否登陆

	private static final SshRemoteServerOperateForMtr INSTANCE = new SshRemoteServerOperateForMtr()
	{

	};

	public static SshRemoteServerOperateForMtr getInstance()
	{
		return INSTANCE;
	}
