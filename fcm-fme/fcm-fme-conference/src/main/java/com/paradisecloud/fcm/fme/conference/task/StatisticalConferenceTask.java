...
public class StatisticalConferenceTask extends Task {
...
//                    conferenceDuration += durationSeconds;
            ...
            FmeBridge fmeBridge = null;
            try {
                fmeBridge = FmeBridgeCache.getInstance().getFmeBridgesByDept(deptId).get(0);
            } catch (Exception e) {
            }
            if (fmeBridge != null) {
                String mcuIp = fmeBridge.getBusiFme().getIp();
