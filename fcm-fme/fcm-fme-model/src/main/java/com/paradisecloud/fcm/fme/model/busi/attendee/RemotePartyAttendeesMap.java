 ...
 * @version V1.0  
 */
public class RemotePartyAttendeesMap extends ConcurrentHashMap<String, Map<String, Attendee>>
{
    ...
    {
        Map<String, Attendee> uuidAttendeeMap = get(attendee.getRemoteParty());
    ...
    public Attendee removeAttendeeByRemotePartyAndUuid(String uri, String uuid)
    {
        Map<String, Attendee> m = get(uri);
        if (m != null)
        {
            Attendee a = m.remove(uuid);
            if (m.isEmpty())
            {
                remove(uri);
            }
