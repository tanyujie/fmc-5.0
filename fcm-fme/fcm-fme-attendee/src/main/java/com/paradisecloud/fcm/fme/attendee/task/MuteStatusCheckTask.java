...
public class MuteStatusCheckTask extends Task {

    private ConferenceContext conferenceContext;

    public MuteStatusCheckTask(String id, long delayInMilliseconds, ConferenceContext conferenceContext) {
    ...
    public void run() {
        if (conferenceContext != null) {
            ...
            if (conferenceContext.getMasterAttendee() != null) {
                Attendee attendee = conferenceContext.getMasterAttendee();
            ...
            if (!guestMixing) {
                for (Long deptId : conferenceContext.getCascadeAttendeesMap().keySet()) {
                    List<Attendee> attendees = conferenceContext.getCascadeAttendeesMap().get(deptId);
                    if (attendees != null) {
