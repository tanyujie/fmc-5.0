 ...
 * @version V1.0
 */
public class PollingAttendeeOpreationImpl extends PollingAttendeeOpreation
{
     ...
     * @param pollingScheme
     */
    public PollingAttendeeOpreationImpl(ConferenceContext conferenceContext, PollingScheme pollingScheme)
    {
        super(conferenceContext);
        attendeeImportance = pollingScheme.getIsBroadcast() == YesOrNo.YES ? AttendeeImportance.ROUND_BROADCAST : AttendeeImportance.ROUND;
        ...
        if(SplitScreenCreaterMap.isCustomLayoutTemplate(pollingScheme.getLayout())){

            CustomScreenCreater customScreenCreater = fbs.get(0).getDataCache().getSplitScreenCreaterMap().get(pollingScheme.getLayout());
            JSONObject json = customScreenCreater.getLayoutTemplate();
            JSONArray panes = json.getJSONArray("panes");
            if (panes != null && panes.size() > 0) {
                CustomLayout customLayout = new CustomLayout(customScreenCreater.getLayout(), panes.toArray().length, attendeeImportance.getEndValue());
    ...
    public void operate()
    {
        ...
        currentThread = new Thread(()->{
            ...
            while (true)
            {
                try
                {
                    if (isCancel || conferenceContext.isEnd()||conferenceContext.getAttendeeOperation() != PollingAttendeeOpreationImpl.this) {
