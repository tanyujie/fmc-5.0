...
public abstract class AttendeePollingStrategy
{
     ...
     */
    public abstract List<PollingAttendee> parse(ConferenceContext conferenceContext, List<DeptPollingAttendees> deptPollingAttendeesList, SplitScreen splitScreen);
    
    protected List<PollingAttendee> parseSpecifiedRangeAndDeptFirst(ConferenceContext conferenceContext, List<DeptPollingAttendees> deptPollingAttendeesList)
    {
     ...
     * @return List<PollingAttendee>
     */
    protected List<PollingAttendee> parseGlobalPolling(ConferenceContext conferenceContext, Map<String, PollingAttendee> m, SplitScreen splitScreen)
    {
        List<PollingAttendee> pollingAttendeeList = new ArrayList<>();
        if (m == null) {
            m = toMap(pollingAttendeeList);
        }
            ...
            for (Attendee attendeeTemp : conferenceContext.getAttendeesOps()) {
                if (attendeeTemp != null && attendeeTemp.getId() != null) {
                    ...
                    if (attendeeExist != null) {
                        add(pollingAttendeeList, attendeeExist, m, splitScreen);
                    }
                }
            }
        }
     ...
     * @return List<PollingAttendee>
     */
    protected List<PollingAttendee> parseGlobalPollingAndDeptFirst(ConferenceContext conferenceContext, Map<String, PollingAttendee> m, SplitScreen splitScreen)
    {
        List<PollingAttendee> pollingAttendeeList = new ArrayList<>();
        if (m == null) {
            m = toMap(pollingAttendeeList);
        ...
        for (Attendee attendee : new ArrayList<>(conferenceContext.getMasterAttendees()))
        {
            add(pollingAttendeeList, attendee, m, splitScreen);
        }
    ...
    private void add(List<PollingAttendee> pollingAttendeeList, Attendee a, Map<String, PollingAttendee> m, SplitScreen splitScreen)
    {
                ...
                if (downCascadeConferenceContext != null) {
                    ...
                    if (downCascadeAttendeeTempList.size() > 0) {
                        for (BaseAttendee downCascadeAttendeeTemp : downCascadeAttendeeTempList) {
                            ...
                            addToPollingAttendeeList(pollingAttendeeList, a, m);
                        }
                    } else {
                        addToPollingAttendeeList(pollingAttendeeList, a, m);
                    }
    ...
    private void addToPollingAttendeeList(List<PollingAttendee> pollingAttendeeList, Attendee a, Map<String, PollingAttendee> m) {
        ...
        if (needAdd) {
            PollingAttendee pollingAttendee = new PollingAttendee(a);
            m.put(a.getId(), pollingAttendee);
            pollingAttendeeList.add(pollingAttendee);
        }
    }
}
