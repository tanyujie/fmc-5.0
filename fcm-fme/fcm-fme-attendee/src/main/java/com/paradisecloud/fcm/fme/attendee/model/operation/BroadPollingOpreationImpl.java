 ...
 * @version V1.0
 */
public class BroadPollingOpreationImpl extends PollingAttendeeOpreation
{
     ...
     */
    public BroadPollingOpreationImpl(ConferenceContext conferenceContext)
    {
        ...
        attendeeImportance = AttendeeImportance.ROUND_BROADCAST;
        ...
        this.splitScreen=new OneSplitScreen(attendeeImportance.getEndValue());
    ...
    public void operate()
    {
      //  conferenceContext.setStartRound(true);
        currentThread = new Thread(()->{
            ...
            conferenceContext.setConferenceMode(ConferenceOpsModeEnum.BROAD_POLLING.name());
            WebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(conferenceContext, WebsocketMessageType.MESSAGE_TIP, "会议已进入一分屏广播轮询模式！");
            WebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(conferenceContext, WebsocketMessageType.CONFERENCE_MODEL, ConferenceOpsModeEnum.BROAD_POLLING.name());
            while (true)
            {
                try
                {
                    if (isCancel || conferenceContext.isEnd()||conferenceContext.getAttendeeOperation() != BroadPollingOpreationImpl.this) {
...
//        WebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(conferenceContext, WebsocketMessageType.MESSAGE_TIP, "会议已关闭一分屏广播轮询模式！");
//        WebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(conferenceContext, WebsocketMessageType.CONFERENCE_MODEL, ConferenceOpsModeEnum.DIRECT.name());
    ...
    private void parse()
    {
        ...
        if (isPollingDownCascade) {
            int maxImportance = AttendeeImportance.BROADCAST.getEndValue();
