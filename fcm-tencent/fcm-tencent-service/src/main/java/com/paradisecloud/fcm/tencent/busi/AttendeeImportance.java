...
public enum AttendeeImportance {
    ...
    COMMON(null, null, "分会场") {
        @Override
        public void processAttendeeWebsocketMessage(AttendeeTencent attendee) {
    ...
    MASTER(7500000, 7500000, "主会场") {
        ...
        public void processAttendeeWebsocketMessage(AttendeeTencent attendee) {
            ...
            attendee.setBroadcastStatus(BroadcastStatus.NO.getValue());
            attendee.setRoundRobinStatus(AttendeeRoundRobinStatus.NO.getValue());
            TencentConferenceContext mainConferenceContext = TencentConferenceContextCache.getInstance().get(attendee.getContextKey());
            if (mainConferenceContext != null && mainConferenceContext.getMasterAttendee() != attendee) {
                AttendeeTencent oldMasterAttendee = mainConferenceContext.getMasterAttendee();
                ...
                TencentWebSocketMessagePusher.getInstance().pushConferenceMessageToAll(mainConferenceContext, WebsocketMessageType.MASTER_CHANGED, data);

                StringBuilder messageTip = new StringBuilder();
                messageTip.append("主会场已切换至【").append(attendee.getName()).append("】");
                TencentWebSocketMessagePusher.getInstance().pushConferenceMessageToAll(mainConferenceContext, WebsocketMessageType.MESSAGE_TIP, messageTip);
        ...
        public void processAttendeeWebsocketMessage(AttendeeTencent attendee) {
            ...
            if (attendee.getUpdateMap().size() > 1) {
                ...
                if (mainConferenceContext != null) {
                    ...
                    TencentWebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(mainConferenceContext, WebsocketMessageType.MESSAGE_TIP, messageTip);
                    TencentWebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(mainConferenceContext, WebsocketMessageType.ATTENDEE_UPDATE, attendee.getUpdateMap());
                }
            }
        }
    },
     ...
     */
    public abstract void processAttendeeWebsocketMessage(AttendeeTencent attendee);
