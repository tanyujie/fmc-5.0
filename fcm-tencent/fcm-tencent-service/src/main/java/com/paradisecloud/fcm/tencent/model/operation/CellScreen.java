...
public class CellScreen implements Serializable
{
     ...
     * 分屏单元格需要渲染的参会者
     */
    private List<SmcParitipantsStateRep.ContentDTO> attendees = new ArrayList<>();
    /**
     * 分屏单元格需要渲染的参会者
     */
    private List<AttendeeTencent> attendeeTencents = new ArrayList<>();
    /**
     * 轮询索引下标
     */
     ...
     */
    @JsonIgnore
    private SmcParitipantsStateRep.ContentDTO lastOperationAttendee;
    @JsonIgnore
    private AttendeeTencent lastOperationAttendeeTencent;
    /**
     * 单元格参会者操作枚举选项
     */
    ...
    public void addAttendee(SmcParitipantsStateRep.ContentDTO a)
    {
        attendees.add(a);
    }
    public void addAttendeeTencent(AttendeeTencent a)
    {
        attendeeTencents.add(a);
    }
     ...
     */
    public SmcParitipantsStateRep.ContentDTO getLastOperationAttendee()
    {
        return lastOperationAttendee;
    }

    public AttendeeTencent getLastOperationAttendeeTencent()
    {
        return lastOperationAttendeeTencent;
    }
    /**
     * <p>Set Method   :   lastOperationAttendee Attendee</p>
     * @param lastOperationAttendee
     */
    public void setLastOperationAttendee(SmcParitipantsStateRep.ContentDTO lastOperationAttendee)
    {
     ...
     */
    @JsonIgnore
    public SmcParitipantsStateRep.ContentDTO getPollingAttendee()
    {
    ...
    @JsonIgnore
    public AttendeeTencent getPollingAttendeeTencent()
    {
        if (ObjectUtils.isEmpty(attendeeTencents))
        {
            return null;
        }

        if (pollingIndex >= attendeeTencents.size())
        {
            pollingIndex = 0;
        }
        return attendeeTencents.get(pollingIndex++);
    }

    public List<AttendeeTencent> getAttendeeTencents() {
        return attendeeTencents;
    }

    public void setAttendeeTencents(List<AttendeeTencent> attendeeTencents) {
        this.attendeeTencents = attendeeTencents;
    }
