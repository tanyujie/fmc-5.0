 ...
 * @version V1.0  
 */
public class RemotePartyAttendeesMap extends ConcurrentHashMap<String, Map<String, AttendeeTencent>>
{
    ...
    public void addAttendee(AttendeeTencent attendee)
    {
        if(attendee==null||attendee.getRemoteParty()==null){
            return;
        }
        Map<String, AttendeeTencent> uuidAttendeeMap = get(attendee.getRemoteParty());
    ...
    public AttendeeTencent removeAttendeeByRemotePartyAndUuid(String uri, String uuid)
    {
        if(uri==null||uuid==null){
            return null;
        }
        Map<String, AttendeeTencent> m = get(uri);
        if (m != null)
        {
            AttendeeTencent a = m.remove(uuid);
            if (m.isEmpty())
            {
                remove(uri);
            }
