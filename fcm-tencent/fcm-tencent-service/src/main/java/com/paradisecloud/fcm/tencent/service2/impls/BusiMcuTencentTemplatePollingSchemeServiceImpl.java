...
import com.paradisecloud.fcm.tencent.model.operation.polling.DeptPollingAttendees;
import com.paradisecloud.fcm.tencent.model.operation.polling.PollingAttendee;
import com.paradisecloud.fcm.tencent.model.operation.polling.PollingScheme;
import com.paradisecloud.fcm.tencent.service2.interfaces.IBusiTencentTemplatePollingSchemeService;
...
@Transactional
@Service
public class BusiMcuTencentTemplatePollingSchemeServiceImpl implements IBusiTencentTemplatePollingSchemeService
{
    private Logger logger = LoggerFactory.getLogger(BusiMcuTencentTemplatePollingSchemeServiceImpl.class);

    @Resource
    private BusiMcuTencentTemplatePollingSchemeMapper busiMcuTencentTemplatePollingSchemeMapper;

    @Resource
    private BusiMcuTencentTemplatePollingDeptMapper busiMcuTencentTemplatePollingDeptMapper;

    @Resource
    private BusiMcuTencentTemplatePollingPaticipantMapper busiMcuTencentTemplatePollingPaticipantMapper;
    ...
    {
        return buildBusiTemplatePollingScheme(busiMcuTencentTemplatePollingSchemeMapper.selectBusiMcuTencentTemplatePollingSchemeById(id));
    }

    private ModelBean buildBusiTemplatePollingScheme(BusiMcuTencentTemplatePollingScheme pollingScheme)
    {
        BusiMcuTencentTemplateConference busiTemplateConference = busiMcuTencentTemplateConferenceMapper.selectBusiMcuTencentTemplateConferenceById(pollingScheme.getTemplateConferenceId());
        BusiMcuTencentTemplatePollingPaticipant busiTemplateParticipant = new BusiMcuTencentTemplatePollingPaticipant();
        busiTemplateParticipant.setPollingSchemeId(pollingScheme.getId());
        List<BusiMcuTencentTemplatePollingPaticipant> ps = busiMcuTencentTemplatePollingPaticipantMapper.selectBusiMcuTencentTemplatePollingPaticipantList(busiTemplateParticipant);

        BusiMcuTencentTemplateParticipant con = new BusiMcuTencentTemplateParticipant();
        con.setTemplateConferenceId(pollingScheme.getTemplateConferenceId());
        List<BusiMcuTencentTemplateParticipant> tps = busiMcuTencentTemplateParticipantMapper.selectBusiMcuTencentTemplateParticipantList(con);
        Map<String, BusiMcuTencentTemplateParticipant> tpMap = new HashMap<>();
        for (BusiMcuTencentTemplateParticipant tp : tps)
        {
            tpMap.put(tp.getUuid(), tp);
        }

        TencentConferenceContext cc = null;
        if (busiTemplateConference.getConferenceNumber() != null)
        {
            cc = TencentConferenceContextCache.getInstance().get(busiTemplateConference.getConferenceNumber().toString());
        }
        List<ModelBean> pMbs = new ArrayList<>();
        BusiMcuTencentTemplateParticipant tp = null;
        AttendeeTencent a = null;
        Set<String> invalidPollingAttendeeIds = new HashSet<>();
        for (BusiMcuTencentTemplatePollingPaticipant pp : ps)
        {
        ...
        {
            int c = busiMcuTencentTemplatePollingPaticipantMapper.deleteBusiMcuTencentTemplatePollingPaticipantByAttendeeIds(invalidPollingAttendeeIds.toArray(new String[invalidPollingAttendeeIds.size()]));
            if (c > 0)
            {
        ...
        BusiMcuTencentTemplatePollingDept tdCon = new BusiMcuTencentTemplatePollingDept();
        tdCon.setPollingSchemeId(pollingScheme.getId());
        List<BusiMcuTencentTemplatePollingDept> tds = busiMcuTencentTemplatePollingDeptMapper.selectBusiMcuTencentTemplatePollingDeptList(tdCon);
        List<ModelBean> pds = new ArrayList<>();
        for (BusiMcuTencentTemplatePollingDept busiTemplatePollingDept : tds)
        {
    ...
    {
        return busiMcuTencentTemplatePollingSchemeMapper.selectBusiMcuTencentTemplatePollingSchemeList(busiTemplatePollingScheme);
    }
            ...
            , List<BusiMcuTencentTemplatePollingPaticipant> templatePollingPaticipants)
    {
        ...
        int c = busiMcuTencentTemplatePollingSchemeMapper.insertBusiMcuTencentTemplatePollingScheme(busiTemplatePollingScheme);
        if (c > 0)
        {
            if (pollingStrategy != PollingStrategy.GLOBAL && PollingStrategy.GLOBAL_AND_DEPT_FIRST != pollingStrategy)
            {
                ...
                // 添加模板部门顺序信息
                for (BusiMcuTencentTemplatePollingDept busiTemplateDept : templatePollingDepts)
                {
                    ...
                    busiMcuTencentTemplatePollingDeptMapper.insertBusiMcuTencentTemplatePollingDept(busiTemplateDept);
                }

                Set<String> attendeeIds = new HashSet<>();

                // 添加模板与会者顺序信息
                for (BusiMcuTencentTemplatePollingPaticipant busiTemplateParticipant : templatePollingPaticipants)
                {
                    ...
                    busiMcuTencentTemplatePollingPaticipantMapper.insertBusiMcuTencentTemplatePollingPaticipant(busiTemplateParticipant);
                }
            }
        }
        return c;
            ...
            , List<BusiMcuTencentTemplatePollingPaticipant> templatePollingPaticipants)
    {
        ...
        int c = busiMcuTencentTemplatePollingSchemeMapper.updateBusiMcuTencentTemplatePollingScheme(busiTemplatePollingScheme);
        if (c > 0)
        {
            // 删除轮询方案对应的子表信息
            busiMcuTencentTemplatePollingDeptMapper.deletePollingDeptByPollingSchemeId(busiTemplatePollingScheme.getId());
            busiMcuTencentTemplatePollingPaticipantMapper.deletePollingPaticipantByPollingSchemeId(busiTemplatePollingScheme.getId());
            if (pollingStrategy != PollingStrategy.GLOBAL && PollingStrategy.GLOBAL_AND_DEPT_FIRST != pollingStrategy)
            {
                Assert.isTrue(!ObjectUtils.isEmpty(templatePollingDepts), "当轮询策略为非“全局”和“全局+组织架构优先”时，轮询方案需选定组织架构至少有一个！");
                Assert.isTrue(!ObjectUtils.isEmpty(templatePollingPaticipants), "当轮询策略为非“全局”和“全局+组织架构优先”时，轮询方案需选定的参会至少有一个！");
    ...
    private void updateMemery(BusiMcuTencentTemplatePollingScheme busiTemplatePollingScheme)
    {
        busiTemplatePollingScheme = busiMcuTencentTemplatePollingSchemeMapper.selectBusiMcuTencentTemplatePollingSchemeById(busiTemplatePollingScheme.getId());
        if (YesOrNo.convert(busiTemplatePollingScheme.getEnableStatus()).getBoolean())
        {
            BusiMcuTencentTemplateConference bc = busiMcuTencentTemplateConferenceMapper.selectBusiMcuTencentTemplateConferenceById(busiTemplatePollingScheme.getTemplateConferenceId());
            if (bc.getConferenceNumber() != null)
            {
                TencentConferenceContext conferenceContext = TencentConferenceContextCache.getInstance().get(bc.getConferenceNumber().toString());
    ...
    @Override
    public int updateBusiTemplatePollingSchemes(List<BusiMcuTencentTemplatePollingScheme> templatePollingSchemes)
    {
        if (ObjectUtils.isEmpty(templatePollingSchemes))
        {
            return 0;
        }
        BusiMcuTencentTemplatePollingScheme open = null;
        for (BusiMcuTencentTemplatePollingScheme busiTemplatePollingScheme : templatePollingSchemes)
        {
            busiTemplatePollingScheme.setUpdateTime(new Date());
            busiMcuTencentTemplatePollingSchemeMapper.updateBusiMcuTencentTemplatePollingScheme(busiTemplatePollingScheme);
            if (YesOrNo.convert(busiTemplatePollingScheme.getEnableStatus()).getBoolean())
            {
        ...
        {
            BusiMcuTencentTemplatePollingScheme busiTemplatePollingScheme = busiMcuTencentTemplatePollingSchemeMapper.selectBusiMcuTencentTemplatePollingSchemeById(templatePollingSchemes.get(0).getId());
            BusiMcuTencentTemplateConference bc = busiMcuTencentTemplateConferenceMapper.selectBusiMcuTencentTemplateConferenceById(busiTemplatePollingScheme.getTemplateConferenceId());
            if (bc.getConferenceNumber() != null)
            {
                TencentConferenceContext conferenceContext = TencentConferenceContextCache.getInstance().get(bc.getConferenceNumber().toString());
    ...
    @Override
    public PollingScheme convert(BusiMcuTencentTemplatePollingScheme ps, TencentConferenceContext conferenceContext)
    {
        ...
        BusiMcuTencentTemplatePollingDept con = new BusiMcuTencentTemplatePollingDept();
        con.setPollingSchemeId(ps.getId());
        List<BusiMcuTencentTemplatePollingDept> pollingDepts = busiMcuTencentTemplatePollingDeptMapper.selectBusiMcuTencentTemplatePollingDeptList(con);
        for (BusiMcuTencentTemplatePollingDept busiTemplatePollingDept : pollingDepts)
        {
        ...
        {
            BusiMcuTencentTemplatePollingPaticipant con1 = new BusiMcuTencentTemplatePollingPaticipant();
            con1.setPollingSchemeId(ps.getId());
            List<BusiMcuTencentTemplatePollingPaticipant> pps = busiMcuTencentTemplatePollingPaticipantMapper.selectBusiMcuTencentTemplatePollingPaticipantList(con1);
            Set<String> invalidPollingAttendeeIds = new HashSet<>();
            Set<String> repeatPollingAttendeeIds = new HashSet<>();
            for (BusiMcuTencentTemplatePollingPaticipant busiTemplatePollingPaticipant : pps)
            {
                // 根据remoteParty获取参会者
                AttendeeTencent attendee = conferenceContext.getAttendeeById(busiTemplatePollingPaticipant.getAttendeeId());
                if (attendee != null)
                {
    ...
    {
        // 删除轮询方案对应的子表信息
        busiMcuTencentTemplatePollingDeptMapper.deletePollingDeptByPollingSchemeId(id);
        busiMcuTencentTemplatePollingPaticipantMapper.deletePollingPaticipantByPollingSchemeId(id);
        return busiMcuTencentTemplatePollingSchemeMapper.deleteBusiMcuTencentTemplatePollingSchemeById(id);
    }
}
