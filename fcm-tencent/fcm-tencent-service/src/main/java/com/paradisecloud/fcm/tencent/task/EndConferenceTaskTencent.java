...
public class EndConferenceTaskTencent extends TencentDelayTask {

    private static final Logger LOGGER = LoggerFactory.getLogger(EndConferenceTaskTencent.class);

    private Long conferenceId;

    public EndConferenceTaskTencent(String id, long delayInMilliseconds, Long conferenceId) {
    ...
    @Override
    public void run() {
        LOGGER.info("Tencent会议{}结束,处理开始",getId());
        ...
        if (conferenceId != null) {
            ...
            IBusiMcuTencentHistoryConferenceService busiTencentHistoryConferenceService = BeanFactory.getBean(IBusiMcuTencentHistoryConferenceService.class);
            ...
            if (busiHistoryConference != null && busiHistoryConference.getConferenceEndTime() != null) {
                ...
                for (BusiHistoryParticipant busiHistoryParticipant : busiHistoryParticipantList) {
                    if (busiHistoryParticipant.getOutgoingTime() == null || busiHistoryParticipant.getDurationSeconds() == null || busiHistoryParticipant.getDurationSeconds().intValue() == 0) {
                        busiTencentHistoryConferenceService.updateBusiHistoryParticipant(busiHistoryParticipant, busiHistoryConference);
                    }
                }
            }
        }
