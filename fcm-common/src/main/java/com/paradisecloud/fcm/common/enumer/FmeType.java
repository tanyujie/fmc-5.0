...
public enum FmeType
{
    ...
    private static final Map<Integer, FmeType> MAP = new HashMap<>();
    static
    {
        for (FmeType recordType : values())
        {
            MAP.put(recordType.value, recordType);
        }
    }
    
    FmeType(int value, String name)
    {
        this.value = value;
        this.name = name;
    }
    ...
    public static FmeType convert(int value)
    {
        FmeType t = MAP.get(value);
        if (t == null)
        {
