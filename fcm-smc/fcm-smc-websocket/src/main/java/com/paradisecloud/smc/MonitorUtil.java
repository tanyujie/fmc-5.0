...
public class MonitorUtil {
    ...
    private static Map<String, SMCWebsocketClient> monitorWebSocketMap=new ConcurrentHashMap<>();
    ...
    public void start(int monitorInterval,SMCWebsocketClient smcWebsocketClient) {
        monitorInterval = monitorInterval;

        monitorWebSocketMap.put(monitorName, smcWebsocketClient);
    ...
    private Runnable monitorAliveRunnable(){
        return ()->{
            for (Map.Entry<String, SMCWebsocketClient> entry : monitorWebSocketMap.entrySet()) {
                try {
