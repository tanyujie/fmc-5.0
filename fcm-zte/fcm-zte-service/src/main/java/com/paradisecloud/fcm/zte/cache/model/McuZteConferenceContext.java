...
import com.paradisecloud.fcm.zte.attendee.model.operation.PollingAttendeeOperation;
import com.paradisecloud.fcm.zte.cache.AttendeeCountingStatistics;
import com.paradisecloud.fcm.zte.cache.api.ConferenceControlApi;
import com.paradisecloud.fcm.zte.model.busi.attendee.*;
import com.paradisecloud.fcm.zte.model.busi.cascade.CascadeForMcuZte;
import com.paradisecloud.fcm.zte.model.busi.cascade.UpCascadeForMcuZte;
 ...
 * @version V1.0
 */
public class McuZteConferenceContext extends BaseConferenceContext<AttendeeForMcuZte> implements Serializable
{
     ...
     */
    @JsonIgnore
    private volatile Map<String, AttendeeForMcuZte> uuidAttendeesMap = new ConcurrentHashMap<>();
    ...
    @JsonIgnore
    private volatile Map<String, AttendeeForMcuZte> deletedParticipantMap = new ConcurrentHashMap<>();

    @JsonIgnore
    private volatile McuZteSyncInformation syncInformation;
    ...
    public McuZteConferenceContext(McuZteBridge mcuZteBridge) {
        this.mcuZteBridge = mcuZteBridge;
        setMcuTypeObject(McuType.MCU_ZTE);
    ...
    public McuZteBridge getMcuZteBridge() {
        return mcuZteBridge;
    }

    @JsonIgnore
    public BusiMcuZte getBusiMcuZte() {
        return mcuZteBridge.getBusiMcuZte();
    }
    ...
    public boolean hasPresent() {
        if (StringUtils.hasText(this.presentAttendeeId)) {
            AttendeeForMcuZte AttendeeForMcuZte = getAttendeeById(this.presentAttendeeId);
            if (AttendeeForMcuZte != null && AttendeeForMcuZte.isMeetingJoined()) {
                return true;
            }
        }
        return false;
    ...
    public void addUuidAttendee(AttendeeForMcuZte AttendeeForMcuZte) {
        if (StringUtils.hasText(AttendeeForMcuZte.getParticipantUuid())) {
            this.uuidAttendeesMap.put(AttendeeForMcuZte.getParticipantUuid(), AttendeeForMcuZte);
        }
    }

    public void removeUuidAttendee(AttendeeForMcuZte AttendeeForMcuZte) {
        if (StringUtils.hasText(AttendeeForMcuZte.getParticipantUuid())) {
            this.uuidAttendeesMap.remove(AttendeeForMcuZte.getParticipantUuid());
        }
    }

    public AttendeeForMcuZte getAttendeeByUuid(String epUserId) {
    ...
    public McuZteSyncInformation getSyncInformation() {
        return syncInformation;
    }

    public void setSyncInformation(McuZteSyncInformation syncInformation) {
     ...
     * 主会场与会者
     */
    private volatile AttendeeForMcuZte masterAttendee;
     ...
     * 预约任务的ID
     */
    private BusiMcuZteConferenceAppointment conferenceAppointment;
     ...
     * 会议主导方的参会者
     */
    private volatile List<AttendeeForMcuZte> attendees = new ArrayList<>();

    /**
     * 所有级联子会议的主会场集合
     */
    private volatile List<AttendeeForMcuZte> masterAttendees = new ArrayList<>();

    /**
     * MCU自身充当的参会者
     */
    private volatile List<McuAttendeeForMcuZte> mcuAttendees = new ArrayList<>();
     ...
     * 级联方的参会者映射表，key为部门ID
     */
    private volatile Map<Long, List<AttendeeForMcuZte>> cascadeAttendeesMap = new ConcurrentHashMap<>();
     ...
     */
    @JsonIgnore
    private volatile Map<String, AttendeeForMcuZte> attendeeMap = new ConcurrentHashMap<>();

    /**
     * 直播终端列表
     */
    @JsonIgnore
    private volatile List<TerminalAttendeeForMcuZte> liveTerminals = new ArrayList<>();

    /**
     * 直播终端Map key:sn
     */
    private volatile Map<String, TerminalAttendeeForMcuZte> liveTerminalMap = new ConcurrentHashMap();

    @JsonIgnore
    private volatile RemotePartyAttendeesMapForMcuZte remotePartyAttendeesMap = new RemotePartyAttendeesMapForMcuZte();

    /**
     * 下级级联信息
     */
    @JsonIgnore
    private CascadeForMcuZte cascade;

    /**
     * 上级级联信息
     */
    @JsonIgnore
    private UpCascadeForMcuZte upCascade;

    /**
     * MCU终端ID映射
     */
    @JsonIgnore
    private volatile Map<Long, TerminalAttendeeForMcuZte> terminalAttendeeMap = new ConcurrentHashMap<>();

    /**
     * 终端ID映射
     */
    @JsonIgnore
    private volatile Map<Long, AttendeeForMcuZte> terminalAttendeeAllMap = new ConcurrentHashMap<>();
...
//    private volatile SyncInformation syncInformation;
     ...
     * @param attendee
     */
    public synchronized void addAttendee(AttendeeForMcuZte attendee)
    {
        ...
        if (attendee instanceof TerminalAttendeeForMcuZte)
        {
            ...
            TerminalAttendeeForMcuZte old = terminalAttendeeMap.get(terminalAttendee.getTerminalId());
            if (old != null)
            {
    ...
    public void addAttendeeToIdMap(AttendeeForMcuZte attendee)
    {
        this.attendeeMap.put(attendee.getId(), attendee);
    }

    public void addAttendeeToRemotePartyMap(AttendeeForMcuZte attendee)
    {
    ...
    public Map<String, AttendeeForMcuZte> getAttendeeMapByUri(String remoteParty) {
        return remotePartyAttendeesMap.get(remoteParty);
    }

    public void addAttendeeToTerminalAllMap(AttendeeForMcuZte attendee) {
        if (attendee.getTerminalId() != null) {
            terminalAttendeeAllMap.put(attendee.getTerminalId(), attendee);
        }
    }

    public void removeAttendeeFromTerminalAllMap(AttendeeForMcuZte attendee) {
        if (attendee.getTerminalId() != null) {
    ...
    @Override
    public AttendeeForMcuZte getAttendeeByTerminalId(Long terminalId) {
        if (terminalId != null) {
     ...
     */
    @Override
    public void updateAttendeeToRemotePartyMap(String oldRemoteParty, AttendeeForMcuZte attendee) {
        if (StringUtils.hasText(oldRemoteParty)) {
            Map<String, AttendeeForMcuZte> uuidAttendeeMap = remotePartyAttendeesMap.get(oldRemoteParty);
     ...
     */
    @Override
    public AttendeeForMcuZte getMasterAttendee()
    {
        return masterAttendee;
    }
     ...
     */
    public synchronized void setMasterAttendee(AttendeeForMcuZte masterAttendee)
    {
        ...
        if (this.masterAttendee != null)
        {
            ...
            else
            {
                ...
                {
                    List<AttendeeForMcuZte> as = this.cascadeAttendeesMap.get(this.masterAttendee.getDeptId());
                    if (as != null)
                    {
        ...
        if (masterAttendee.getDeptId() != deptId.longValue())
        {
            ...
            {
                List<AttendeeForMcuZte> as = this.cascadeAttendeesMap.get(masterAttendee.getDeptId());
     ...
     * @param masterAttendee void
     */
    public void addMasterAttendee(AttendeeForMcuZte masterAttendee)
    {
     ...
     */
    @Override
    public List<AttendeeForMcuZte> getMasterAttendees()
    {
        return masterAttendees;
    }
     ...
     */
    @Override
    public List<McuAttendeeForMcuZte> getMcuAttendees()
    {
        return mcuAttendees;
    }
     ...
     * @param mcuAttendees
     */
    public void setMcuAttendees(List<McuAttendeeForMcuZte> mcuAttendees)
    {
     ...
     * @param attendee
     */
    public void addMcuAttendee(McuAttendeeForMcuZte attendee)
    {
        attendee.setConferenceNumber(conferenceNumber);
        attendee.setContextKey(getContextKey());
     ...
     * @param mcuAttendee
     */
    public void removeMcuAttendee(McuAttendeeForMcuZte mcuAttendee) {
     ...
     */
    public void putMcuAttendee(McuAttendeeForMcuZte fmeAttendee)
    {
        if (fmeAttendee == null)
        {
            return;
     ...
     * @param fmeAttendee void
     */
    public void putUpMcuAttendee(UpMcuAttendeeForMcuZte fmeAttendee)
    {
        if (fmeAttendee == null)
        {
            return;
        ...
        if (upCascade == null)
        {
            upCascade = new UpCascadeForMcuZte();
        }
        upCascade.add(fmeAttendee);
    }

    public UpMcuAttendeeForMcuZte getUpMcuAttendee(String conferenceNumber)
    {
        if (upCascade == null)
        {
            return null;
    ...
    public UpMcuAttendeeForMcuZte removeUpMcuAttendee(String conferenceNumber)
    {
        if (upCascade == null)
        {
            return null;
    ...
    public McuAttendeeForMcuZte getMcuAttendee(String conferenceNumber)
    {
        if (cascade == null)
        {
            return null;
    ...
    public McuAttendeeForMcuZte removeMcuAttendee(String conferenceNumber)
    {
        if (cascade == null)
        {
            return null;
     ...
     * @return cascade
     */
    public CascadeForMcuZte getCascade()
    {
        return cascade;
    }
     ...
     * @return upCascade
     */
    public UpCascadeForMcuZte getUpCascade()
    {
        return upCascade;
    }
     ...
     */
    @Override
    public Map<Long, List<AttendeeForMcuZte>> getCascadeAttendeesMap()
    {
        return cascadeAttendeesMap;
    }
     ...
     */
    public void setCascadeAttendeesMap(Map<Long, List<AttendeeForMcuZte>> cascadeAttendeesMap)
    {
     ...
     */
    public void addCascadeAttendee(AttendeeForMcuZte attendee)
    {
        attendee.setContextKey(getContextKey());
        List<AttendeeForMcuZte> cascadeAttendees = cascadeAttendeesMap.get(attendee.getDeptId());
    ...
    public Map<String, AttendeeForMcuZte> getUuidAttendeeMapByUri(String remoteParty)
    {
        return remotePartyAttendeesMap.get(remoteParty);
    }
     ...
     */
    @Override
    public AttendeeForMcuZte getAttendeeById(String attendeeId)
    {
     ...
     * @param id
     */
    public synchronized AttendeeForMcuZte removeAttendeeById(String id)
    {
        AttendeeForMcuZte attendee = this.attendeeMap.remove(id);
        if (attendee != null)
        {
            ...
            List<AttendeeForMcuZte> as = cascadeAttendeesMap.get(attendee.getDeptId());
            if (as != null)
            {
                as.remove(attendee);
            }
            ...
            if (attendee instanceof TerminalAttendeeForMcuZte)
            {
                TerminalAttendeeForMcuZte ta = (TerminalAttendeeForMcuZte)attendee;
     ...
     * @param attendeeMap
     */
    public void setAttendeeMap(Map<String, AttendeeForMcuZte> attendeeMap)
    {
     ...
     */
    @Override
    public Map<Long, TerminalAttendeeForMcuZte> getTerminalAttendeeMap()
    {
        return terminalAttendeeMap;
    }
     ...
     */
    public void setTerminalAttendeeMap(Map<Long, TerminalAttendeeForMcuZte> terminalAttendeeMap)
    {
     ...
     */
    public Long getRecallAttendeeBeginTime(AttendeeForMcuZte attendee)
    {
     ...
     * @return recallAttendeeBeginTimeMap
     */
    public Long removeRecallAttendeeBeginTime(AttendeeForMcuZte attendee)
    {
     ...
     */
    public void addRecallAttendeeBeginTime(AttendeeForMcuZte attendee)
    {
    ...
    public boolean equals(Object obj)
    {
        ...
        McuZteConferenceContext other = (McuZteConferenceContext) obj;
        if (id == null)
        {
...
//    }
     ...
     */
    @Override
    public BusiMcuZteConferenceAppointment getConferenceAppointment()
    {
        return conferenceAppointment;
    }
     ...
     */
    public void setConferenceAppointment(BusiMcuZteConferenceAppointment conferenceAppointment)
    {
...
//    }
     ...
     */
    @Override
    public List<TerminalAttendeeForMcuZte> getLiveTerminals()
    {
        return liveTerminals;
    }
     ...
     */
    @Override
    public TerminalAttendeeForMcuZte getLiveTerminal(String sn)
    {
     ...
     * @param terminal
     */
    public void addLiveTerminal(TerminalAttendeeForMcuZte terminal)
    {
    ...
    public String getMcuIp() {
        if (mcuZteBridge != null) {
            if (StringUtils.hasText(mcuZteBridge.getBusiMcuZte().getProxyHost())) {
                return mcuZteBridge.getBusiMcuZte().getProxyHost();
            }
            if (StringUtils.hasText(mcuZteBridge.getBusiMcuZte().getIp())) {
                return mcuZteBridge.getBusiMcuZte().getIp();
            }
        }
        return "";
    }
    ...
    public Integer getMcuPort() {
        if (mcuZteBridge != null) {
            if (mcuZteBridge.getBusiMcuZte().getProxyPort() != null) {
                return mcuZteBridge.getBusiMcuZte().getProxyPort();
            }
            if (mcuZteBridge.getBusiMcuZte().getPort() != null) {
                return mcuZteBridge.getBusiMcuZte().getPort();
            }
        }
        return null;
    }
    ...
    public Integer getMcuCallPort() {
        if (mcuZteBridge != null) {
            if (mcuZteBridge.getBusiMcuZte().getCallPort() != null) {
                return mcuZteBridge.getBusiMcuZte().getCallPort();
            }
        }
        return null;
    }
    ...
    public AttendeeForMcuZte getDeletedParticipant(String uuid) {
        if(Strings.isBlank(uuid)){
            return null;
        }
        return deletedParticipantMap.get(uuid);
    }

    public AttendeeForMcuZte removeDeletedParticipant(String uuid) {
        if(Strings.isBlank(uuid)){
            return null;
        }
        return deletedParticipantMap.remove(uuid);
    }

    public void setDeletedParticipant(String uuid,AttendeeForMcuZte attendeeForMcuZte) {
        this.deletedParticipantMap.put(uuid,attendeeForMcuZte);
    }
