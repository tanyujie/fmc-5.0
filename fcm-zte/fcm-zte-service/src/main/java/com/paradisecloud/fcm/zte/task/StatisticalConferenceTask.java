...
public class StatisticalConferenceTask extends DelayTask {
    ...
    @Override
    public void run() {
        LOGGER.info("MCU_Zte会议统计开始。ID:" + getId());
...
//                    conferenceDuration += durationSeconds;
            ...
            try {
                mcuZteBridge = McuZteBridgeCache.getInstance().getAvailableMcuZteBridgesByDept(deptId).getMasterMcuZteBridge();
            } catch (Exception e) {
            }
            if (mcuZteBridge != null) {
                String mcuIp = mcuZteBridge.getBusiMcuZte().getIp();
