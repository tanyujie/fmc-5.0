...
import com.paradisecloud.fcm.zte.service.interfaces.IBusiRecordsForMcuZteService;
 ...
 */
@Service
public class BusiRecordsServiceForMcuZteImpl implements IBusiRecordsForMcuZteService {
    ...
    @Resource
    private IBusiMcuZteConferenceService iBusiMcuZteConferenceService;
    ...
    public int updateBusiRecords(boolean recording, String contextKey) {
        ...
        boolean success = iBusiMcuZteConferenceService.updateCallRecordStatus(contextKey, recording);
    ...
    public int getRecordSpaceStatus() {
        ...
        Collection<McuZteConferenceContext> values = McuZteConferenceContextCache.getInstance().values();
        int i = 1;
        if (values != null && values.size() > 0) {
            for (McuZteConferenceContext value : values) {
                if (value.isRecorded()) {
                    i++;
                }
            }
     ...
     * @param path
     */
    private void readRecordFileName(McuZteConferenceContext conferenceContext, BusiRecords busiRecords, String path) {
        ...
        if (file.exists()) {
            ...
            if (null != files && files.size() > 0) {
                ...
                busiRecords.setFileName(new StringBuffer(conferenceContext.getConferenceNumber()).append("_").append(conferenceContext.getName()).append("_录制文件").append(files.size()).toString());
