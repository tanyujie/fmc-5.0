...
public class McuZteSendConferenceInfoToTerminalTask extends DelayTask {

    private McuZteConferenceContext conferenceContext;
    private Long newPresenter;

    public McuZteSendConferenceInfoToTerminalTask(String id, long delayInMilliseconds, McuZteConferenceContext conferenceContext) {
        super("send_c_i_" + id, delayInMilliseconds);
        this.conferenceContext = conferenceContext;
    }

    public McuZteSendConferenceInfoToTerminalTask(String id, long delayInMilliseconds, McuZteConferenceContext conferenceContext, Long newPresenter) {
    ...
    public void run() {
        if (conferenceContext != null) {
            ...
            if (busiUserTerminal != null) {
                AttendeeForMcuZte terminalAttendee = conferenceContext.getAttendeeByTerminalId(busiUserTerminal.getTerminalId());
            ...
            if (busiUserTerminalPresenter != null) {
                AttendeeForMcuZte terminalAttendeePresenter = conferenceContext.getAttendeeByTerminalId(busiUserTerminalPresenter.getTerminalId());
            ...
            if (newPresenter != null && newPresenter.longValue() != presenter.longValue()) {
                ...
                if (busiUserTerminalNewPresenter != null) {
                    AttendeeForMcuZte terminalAttendeePresenter = conferenceContext.getAttendeeByTerminalId(busiUserTerminalNewPresenter.getTerminalId());
                    if (terminalAttendeePresenter != null && terminalAttendeePresenter.getMeetingStatus() == AttendeeMeetingStatus.IN.getValue()) {
                        BusiTerminal busiTerminal = TerminalCache.getInstance().get(terminalAttendeePresenter.getTerminalId());
                        if (StringUtils.isNotEmpty(busiTerminal.getSn())) {
                            if (busiTerminal.getMqttOnlineStatus() == TerminalOnlineStatus.ONLINE.getValue()) {
