...
public class StatisticalConferenceTask extends DelayTask {

    private static final Logger LOGGER = LoggerFactory.getLogger(StatisticalConferenceTask.class);

    private Long deptId;

    public StatisticalConferenceTask(String id, long delayInMilliseconds, Long deptId) {
    ...
    @Override
    public void run() {
        LOGGER.info("MCU_PLC会议统计开始。ID:" + getId());
...
//                    conferenceDuration += durationSeconds;
            ...
            try {
                mcuKdcBridge = McuKdcBridgeCache.getInstance().getAvailableMcuKdcBridgesByDept(deptId).getMasterMcuKdcBridge();
            } catch (Exception e) {
            }
            if (mcuKdcBridge != null) {
                String mcuIp = mcuKdcBridge.getBusiMcuKdc().getIp();
                updateCallNumDate(deptId, mcuIp);
            }
        }
    }
