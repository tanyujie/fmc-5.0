 ...
 */
@Service
public class BusiRecordsServiceForMcuKdcImpl implements IBusiRecordsForMcuKdcService {
    ...
    @Resource
    private IBusiMcuKdcConferenceService iBusiMcuKdcConferenceService;
    ...
    public int updateBusiRecords(boolean recording, String contextKey) {
        ...
        boolean success = iBusiMcuKdcConferenceService.updateCallRecordStatus(contextKey, recording);
    ...
    public int getRecordSpaceStatus() {
        ...
        Collection<McuKdcConferenceContext> values = McuKdcConferenceContextCache.getInstance().values();
        int i = 1;
        if (values != null && values.size() > 0) {
            for (McuKdcConferenceContext value : values) {
                if (value.isRecorded()) {
                    i++;
                }
            }
     ...
     * @param path
     */
    private void readRecordFileName(String conferenceNumber, McuKdcConferenceContext conferenceContext, BusiRecords busiRecords, String path) {
