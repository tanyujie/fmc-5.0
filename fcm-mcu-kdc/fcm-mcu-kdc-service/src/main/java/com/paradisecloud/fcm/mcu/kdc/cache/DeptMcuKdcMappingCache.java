 ...
 * @version V1.0  
 */
public class DeptMcuKdcMappingCache extends JavaCache<Long, BusiMcuKdcDept>
{
    ...
    private static final DeptMcuKdcMappingCache INSTANCE = new DeptMcuKdcMappingCache();
     ...
     */
    private DeptMcuKdcMappingCache()
    {
    }
    
    public static DeptMcuKdcMappingCache getInstance()
    {
        return INSTANCE;
    }
    
    public BusiMcuKdcDept getBindMcu(Long key)
    {
        BusiMcuKdcDept busiMcuKdcDept = super.get(key);
    ...
    public boolean isBindSameMcu(long... deptIds)
    {
        ...
        BusiMcuKdcDept last = getBindMcu(deptIds[0]);
        for (int i = 1; i < deptIds.length; i++)
        {
            BusiMcuKdcDept busiMcuKdcDept = getBindMcu(deptIds[i]);
            if (!(McuKdcType.convert(busiMcuKdcDept.getMcuType()) == McuKdcType.convert(last.getMcuType()) && busiMcuKdcDept.getMcuId().longValue() == last.getMcuId().longValue()))
            {
                return false;
            }
        }
     ...
     * @return int
     */
    public int getBindDeptCount(McuKdcType mcuXdType, long mcuXdId)
    {
        int c = 0;
        for (BusiMcuKdcDept busiMcuKdcDept : values())
        {
            if (McuKdcType.convert(busiMcuKdcDept.getMcuType()) == mcuXdType && busiMcuKdcDept.getMcuId().longValue() == mcuXdId)
            {
                c++;
            }
        }
