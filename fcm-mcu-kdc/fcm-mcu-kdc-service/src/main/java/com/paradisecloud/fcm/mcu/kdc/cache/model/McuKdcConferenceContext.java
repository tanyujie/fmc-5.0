...
import com.paradisecloud.fcm.mcu.kdc.attendee.model.operation.PollingAttendeeOperation;
import com.paradisecloud.fcm.mcu.kdc.cache.AttendeeCountingStatistics;
import com.paradisecloud.fcm.mcu.kdc.cache.api.ConferenceControlApi;
import com.paradisecloud.fcm.mcu.kdc.model.busi.attendee.*;
import com.paradisecloud.fcm.mcu.kdc.model.busi.cascade.CascadeForMcuKdc;
import com.paradisecloud.fcm.mcu.kdc.model.busi.cascade.UpCascadeForMcuKdc;
 ...
 * @version V1.0
 */
public class McuKdcConferenceContext extends BaseConferenceContext<AttendeeForMcuKdc> implements Serializable
{
     ...
     */
    @JsonIgnore
    private volatile Map<String, AttendeeForMcuKdc> uuidAttendeesMap = new ConcurrentHashMap<>();
    ...
    @JsonIgnore
    private volatile McuKdcSyncInformation syncInformation;
    ...
    public McuKdcConferenceContext(McuKdcBridge mcuKdcBridge) {
        this.mcuKdcBridge = mcuKdcBridge;
        setMcuTypeObject(McuType.MCU_KDC);
    ...
    private void init() {
        ...
        setMcuId(mcuKdcBridge.getBusiMcuKdc().getId());
    }
    ...
    public McuKdcBridge getMcuKdcBridge() {
        return mcuKdcBridge;
    }

    @JsonIgnore
    public BusiMcuKdc getBusiMcuKdc() {
        return mcuKdcBridge.getBusiMcuKdc();
    }
    ...
    public boolean hasPresent() {
        if (StringUtils.hasText(this.presentAttendeeId)) {
            AttendeeForMcuKdc attendeeForMcuKdc = getAttendeeById(this.presentAttendeeId);
            if (attendeeForMcuKdc != null && attendeeForMcuKdc.isMeetingJoined()) {
                return true;
            }
        }
        return false;
    ...
    public void addUuidAttendee(AttendeeForMcuKdc attendeeForMcuKdc) {
        if (StringUtils.hasText(attendeeForMcuKdc.getParticipantUuid())) {
            this.uuidAttendeesMap.put(attendeeForMcuKdc.getParticipantUuid(), attendeeForMcuKdc);
        }
    }

    public void removeUuidAttendee(AttendeeForMcuKdc attendeeForMcuKdc) {
        if (StringUtils.hasText(attendeeForMcuKdc.getParticipantUuid())) {
            this.uuidAttendeesMap.remove(attendeeForMcuKdc.getParticipantUuid());
        }
    }

    public AttendeeForMcuKdc getAttendeeByUuid(String epUserId) {
    ...
    public McuKdcSyncInformation getSyncInformation() {
        return syncInformation;
    }

    public void setSyncInformation(McuKdcSyncInformation syncInformation) {
     ...
     * 主会场与会者
     */
    private volatile AttendeeForMcuKdc masterAttendee;
     ...
     * 预约任务的ID
     */
    private BusiMcuKdcConferenceAppointment conferenceAppointment;
     ...
     * 会议主导方的参会者
     */
    private volatile List<AttendeeForMcuKdc> attendees = new ArrayList<>();

    /**
     * 所有级联子会议的主会场集合
     */
    private volatile List<AttendeeForMcuKdc> masterAttendees = new ArrayList<>();

    /**
     * MCU自身充当的参会者
     */
    private volatile List<McuAttendeeForMcuKdc> mcuAttendees = new ArrayList<>();
     ...
     * 级联方的参会者映射表，key为部门ID
     */
    private volatile Map<Long, List<AttendeeForMcuKdc>> cascadeAttendeesMap = new ConcurrentHashMap<>();
     ...
     */
    @JsonIgnore
    private volatile Map<String, AttendeeForMcuKdc> attendeeMap = new ConcurrentHashMap<>();

    /**
     * 直播终端列表
     */
    @JsonIgnore
    private volatile List<TerminalAttendeeForMcuKdc> liveTerminals = new ArrayList<>();

    /**
     * 直播终端Map key:sn
     */
    private volatile Map<String, TerminalAttendeeForMcuKdc> liveTerminalMap = new ConcurrentHashMap();

    @JsonIgnore
    private volatile RemotePartyAttendeesMapForMcuKdc remotePartyAttendeesMap = new RemotePartyAttendeesMapForMcuKdc();

    /**
     * 下级级联信息
     */
    @JsonIgnore
    private CascadeForMcuKdc cascade;

    /**
     * 上级级联信息
     */
    @JsonIgnore
    private UpCascadeForMcuKdc upCascade;

    /**
     * MCU终端ID映射
     */
    @JsonIgnore
    private volatile Map<Long, TerminalAttendeeForMcuKdc> terminalAttendeeMap = new ConcurrentHashMap<>();

    /**
     * 终端ID映射
     */
    @JsonIgnore
    private volatile Map<Long, AttendeeForMcuKdc> terminalAttendeeAllMap = new ConcurrentHashMap<>();
...
//    private volatile SyncInformation syncInformation;
     ...
     */
    @Override
    public List<AttendeeForMcuKdc> getAttendees()
    {
        return attendees;
    }
     ...
     * @param attendees
     */
    public void setAttendees(List<AttendeeForMcuKdc> attendees)
    {
     ...
     * @param attendee
     */
    public synchronized void addAttendee(AttendeeForMcuKdc attendee)
    {
        ...
        if (attendee instanceof TerminalAttendeeForMcuKdc)
        {
            ...
            TerminalAttendeeForMcuKdc old = terminalAttendeeMap.get(terminalAttendee.getTerminalId());
            if (old != null)
            {
    ...
    public void addAttendeeToIdMap(AttendeeForMcuKdc attendee)
    {
        this.attendeeMap.put(attendee.getId(), attendee);
    }

    public void addAttendeeToRemotePartyMap(AttendeeForMcuKdc attendee)
    {
    ...
    public Map<String, AttendeeForMcuKdc> getAttendeeMapByUri(String remoteParty) {
        return remotePartyAttendeesMap.get(remoteParty);
    }

    public void addAttendeeToTerminalAllMap(AttendeeForMcuKdc attendee) {
        if (attendee.getTerminalId() != null) {
            terminalAttendeeAllMap.put(attendee.getTerminalId(), attendee);
        }
    }

    public void removeAttendeeFromTerminalAllMap(AttendeeForMcuKdc attendee) {
        if (attendee.getTerminalId() != null) {
    ...
    @Override
    public AttendeeForMcuKdc getAttendeeByTerminalId(Long terminalId) {
        if (terminalId != null) {
     ...
     * @param attendee
     */
    public void updateAttendeeToRemotePartyMap(String oldRemoteParty, AttendeeForMcuKdc attendee) {
        if (StringUtils.hasText(oldRemoteParty)) {
            Map<String, AttendeeForMcuKdc> uuidAttendeeMap = remotePartyAttendeesMap.get(oldRemoteParty);
     ...
     */
    @Override
    public AttendeeForMcuKdc getMasterAttendee()
    {
        return masterAttendee;
    }
     ...
     */
    public synchronized void setMasterAttendee(AttendeeForMcuKdc masterAttendee)
    {
        ...
        if (this.masterAttendee != null)
        {
            ...
            else
            {
                ...
                {
                    List<AttendeeForMcuKdc> as = this.cascadeAttendeesMap.get(this.masterAttendee.getDeptId());
                    if (as != null)
                    {
        ...
        if (masterAttendee.getDeptId() != deptId.longValue())
        {
            ...
            {
                List<AttendeeForMcuKdc> as = this.cascadeAttendeesMap.get(masterAttendee.getDeptId());
     ...
     * @param masterAttendee void
     */
    public void addMasterAttendee(AttendeeForMcuKdc masterAttendee)
    {
     ...
     */
    @Override
    public List<AttendeeForMcuKdc> getMasterAttendees()
    {
        return masterAttendees;
    }
     ...
     */
    @Override
    public List<McuAttendeeForMcuKdc> getMcuAttendees()
    {
        return mcuAttendees;
    }
     ...
     * @param mcuAttendees
     */
    public void setMcuAttendees(List<McuAttendeeForMcuKdc> mcuAttendees)
    {
     ...
     * @param attendee
     */
    public void addMcuAttendee(McuAttendeeForMcuKdc attendee)
    {
        attendee.setConferenceNumber(conferenceNumber);
        attendee.setContextKey(getContextKey());
     ...
     * @param mcuAttendee
     */
    public void removeMcuAttendee(McuAttendeeForMcuKdc mcuAttendee) {
     ...
     */
    public void putMcuAttendee(McuAttendeeForMcuKdc fmeAttendee)
    {
        if (fmeAttendee == null)
        {
            return;
     ...
     * @param fmeAttendee void
     */
    public void putUpMcuAttendee(UpMcuAttendeeForMcuKdc fmeAttendee)
    {
        if (fmeAttendee == null)
        {
            return;
        ...
        if (upCascade == null)
        {
            upCascade = new UpCascadeForMcuKdc();
        }
        upCascade.add(fmeAttendee);
    }

    public UpMcuAttendeeForMcuKdc getUpMcuAttendee(String conferenceNumber)
    {
        if (upCascade == null)
        {
            return null;
    ...
    public UpMcuAttendeeForMcuKdc removeUpMcuAttendee(String conferenceNumber)
    {
        if (upCascade == null)
        {
            return null;
    ...
    public McuAttendeeForMcuKdc getMcuAttendee(String conferenceNumber)
    {
        if (cascade == null)
        {
            return null;
    ...
    public McuAttendeeForMcuKdc removeMcuAttendee(String conferenceNumber)
    {
        if (cascade == null)
        {
            return null;
     ...
     * @return cascade
     */
    public CascadeForMcuKdc getCascade()
    {
        return cascade;
    }
     ...
     * @return upCascade
     */
    public UpCascadeForMcuKdc getUpCascade()
    {
        return upCascade;
    }
     ...
     */
    @Override
    public Map<Long, List<AttendeeForMcuKdc>> getCascadeAttendeesMap()
    {
        return cascadeAttendeesMap;
    }
     ...
     */
    public void setCascadeAttendeesMap(Map<Long, List<AttendeeForMcuKdc>> cascadeAttendeesMap)
    {
     ...
     */
    public void addCascadeAttendee(AttendeeForMcuKdc attendee)
    {
        attendee.setContextKey(getContextKey());
        List<AttendeeForMcuKdc> cascadeAttendees = cascadeAttendeesMap.get(attendee.getDeptId());
     ...
     */
    @Override
    public AttendeeForMcuKdc getAttendeeById(String attendeeId)
    {
     ...
     * @param id
     */
    public synchronized AttendeeForMcuKdc removeAttendeeById(String id)
    {
        AttendeeForMcuKdc attendee = this.attendeeMap.remove(id);
        if (attendee != null)
        {
            ...
            List<AttendeeForMcuKdc> as = cascadeAttendeesMap.get(attendee.getDeptId());
            if (as != null)
            {
                as.remove(attendee);
            }
            ...
            if (attendee instanceof TerminalAttendeeForMcuKdc)
            {
                TerminalAttendeeForMcuKdc ta = (TerminalAttendeeForMcuKdc)attendee;
     ...
     * @param attendeeMap
     */
    public void setAttendeeMap(Map<String, AttendeeForMcuKdc> attendeeMap)
    {
     ...
     */
    @Override
    public Map<Long, TerminalAttendeeForMcuKdc> getTerminalAttendeeMap()
    {
        return terminalAttendeeMap;
    }
     ...
     */
    public void setTerminalAttendeeMap(Map<Long, TerminalAttendeeForMcuKdc> terminalAttendeeMap)
    {
     ...
     */
    public Long getRecallAttendeeBeginTime(AttendeeForMcuKdc attendee)
    {
     ...
     * @return recallAttendeeBeginTimeMap
     */
    public Long removeRecallAttendeeBeginTime(AttendeeForMcuKdc attendee)
    {
     ...
     */
    public void addRecallAttendeeBeginTime(AttendeeForMcuKdc attendee)
    {
    ...
    public boolean equals(Object obj)
    {
        ...
        McuKdcConferenceContext other = (McuKdcConferenceContext) obj;
        if (id == null)
        {
...
//    }
     ...
     */
    @Override
    public BusiMcuKdcConferenceAppointment getConferenceAppointment()
    {
        return conferenceAppointment;
    }
     ...
     */
    public void setConferenceAppointment(BusiMcuKdcConferenceAppointment conferenceAppointment)
    {
...
//    }
     ...
     */
    @Override
    public List<TerminalAttendeeForMcuKdc> getLiveTerminals()
    {
        return liveTerminals;
    }
     ...
     */
    @Override
    public TerminalAttendeeForMcuKdc getLiveTerminal(String sn)
    {
     ...
     * @param terminal
     */
    public void addLiveTerminal(TerminalAttendeeForMcuKdc terminal)
    {
    ...
    public String getMcuCallIp() {
        if (mcuKdcBridge != null) {
            if (StringUtils.hasText(mcuKdcBridge.getBusiMcuKdc().getIp())) {
                return mcuKdcBridge.getBusiMcuKdc().getIp();
            }
        }
        return null;
    }
    ...
    public Integer getMcuCallPort() {
        if (mcuKdcBridge != null) {
            if (mcuKdcBridge.getBusiMcuKdc().getCallPort() != null) {
                return mcuKdcBridge.getBusiMcuKdc().getCallPort();
            }
        }
        return null;
    }
