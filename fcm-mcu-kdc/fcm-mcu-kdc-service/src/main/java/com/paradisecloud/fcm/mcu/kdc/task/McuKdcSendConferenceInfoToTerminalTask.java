...
public class McuKdcSendConferenceInfoToTerminalTask extends DelayTask {

    private McuKdcConferenceContext conferenceContext;
    private Long newPresenter;

    public McuKdcSendConferenceInfoToTerminalTask(String id, long delayInMilliseconds, McuKdcConferenceContext conferenceContext) {
        super("send_c_i_" + id, delayInMilliseconds);
        this.conferenceContext = conferenceContext;
    }

    public McuKdcSendConferenceInfoToTerminalTask(String id, long delayInMilliseconds, McuKdcConferenceContext conferenceContext, Long newPresenter) {
    ...
    public void run() {
        if (conferenceContext != null) {
            ...
            if (busiUserTerminal != null) {
                AttendeeForMcuKdc terminalAttendee = conferenceContext.getAttendeeByTerminalId(busiUserTerminal.getTerminalId());
            ...
            if (busiUserTerminalPresenter != null) {
                AttendeeForMcuKdc terminalAttendeePresenter = conferenceContext.getAttendeeByTerminalId(busiUserTerminalPresenter.getTerminalId());
            ...
            if (newPresenter != null && newPresenter.longValue() != presenter.longValue()) {
                ...
                if (busiUserTerminalNewPresenter != null) {
                    AttendeeForMcuKdc terminalAttendeePresenter = conferenceContext.getAttendeeByTerminalId(busiUserTerminalNewPresenter.getTerminalId());
                    if (terminalAttendeePresenter != null && terminalAttendeePresenter.getMeetingStatus() == AttendeeMeetingStatus.IN.getValue()) {
                        BusiTerminal busiTerminal = TerminalCache.getInstance().get(terminalAttendeePresenter.getTerminalId());
                        if (StringUtils.isNotEmpty(busiTerminal.getSn())) {
                            if (busiTerminal.getMqttOnlineStatus() == TerminalOnlineStatus.ONLINE.getValue()) {
