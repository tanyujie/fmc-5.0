...
public class EndConferenceTask extends DelayTask {

    private static final Logger LOGGER = LoggerFactory.getLogger(EndConferenceTask.class);

    private Long conferenceId;

    public EndConferenceTask(String id, long delayInMilliseconds, Long conferenceId) {
        super("end_c_" + id, delayInMilliseconds);
    ...
    @Override
    public void run() {
        LOGGER.info("MCU_ZJ会议结束处理开始。ID:" + getId());
            ...
            if (busiHistoryConference != null && busiHistoryConference.getConferenceEndTime() != null) {
                ...
                for (BusiHistoryParticipant busiHistoryParticipant : busiHistoryParticipantList) {
                    if (busiHistoryParticipant.getOutgoingTime() == null || busiHistoryParticipant.getDurationSeconds() == null || busiHistoryParticipant.getDurationSeconds().intValue() == 0) {
                        busiHistoryConferenceForMcuZjService.updateBusiHistoryParticipant(busiHistoryParticipant, busiHistoryConference);
                    }
                }
            }
        }
