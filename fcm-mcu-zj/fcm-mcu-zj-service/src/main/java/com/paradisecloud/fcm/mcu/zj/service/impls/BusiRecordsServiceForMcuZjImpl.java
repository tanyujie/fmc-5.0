 ...
 */
@Service
public class BusiRecordsServiceForMcuZjImpl implements IBusiRecordsForMcuZjService {
    ...
    @Resource
    private IBusiMcuZjConferenceService iBusiMcuZjConferenceService;
    @Resource
    ...
    public int updateBusiRecords(boolean recording, String contextKey) {
        ...
        boolean success = iBusiMcuZjConferenceService.updateCallRecordStatus(contextKey, recording);

        if (success) {
            String coSpaceId = conferenceContext.getCoSpaceId();
            if (recording) {
            } else {
                DownloadRecordFileTask downloadRecordFileTask = new DownloadRecordFileTask(coSpaceId, 30000, conferenceContext.getMcuZjBridge(), coSpaceId, conferenceContext.getDeptId(), conferenceContext.getName());
    ...
    public int getRecordSpaceStatus() {
        ...
        Collection<McuZjConferenceContext> values = McuZjConferenceContextCache.getInstance().values();
        int i = 1;
        if (values != null && values.size() > 0) {
            for (McuZjConferenceContext value : values) {
                if (value.isRecorded()) {
                    i++;
                }
            }
     ...
     * @param path
     */
    private void readRecordFileName(String conferenceNumber, McuZjConferenceContext conferenceContext, BusiRecords busiRecords, String path) {
