...
public class AttendeeUtils
{
    ...
    public static TerminalAttendeeForMcuZj packTerminalAttendee(long terminalId, AttendType attendType, Map<String, Object> businessProperties, int weight, String uuid)
    {
        BusiTerminal terminal = TerminalCache.getInstance().get(terminalId);
        TerminalAttendeeForMcuZj ta = new TerminalAttendeeForMcuZj();
        ...
        {
            ta.setIp(McuZjBridgeCache.getInstance().get(terminal.getZjServerId()).getBusiMcuZj().getIp());
            ta.setRemoteParty(terminal.getCredential() + "@" + ta.getIp());
        }
        else
        {
    ...
    public static TerminalAttendeeForMcuZj packTerminalAttendee(long terminalId)
    {
        return packTerminalAttendee(terminalId, null, null, 1, UUID.randomUUID().toString());
    }
    
    public static TerminalAttendeeForMcuZj packTerminalAttendee(BusiMcuZjTemplateParticipant busiMcuZjTemplateParticipant)
    {
        return packTerminalAttendee(busiMcuZjTemplateParticipant.getTerminalId(), AttendType.convert(busiMcuZjTemplateParticipant.getAttendType()), busiMcuZjTemplateParticipant.getBusinessProperties(), busiMcuZjTemplateParticipant.getWeight(), busiMcuZjTemplateParticipant.getUuid());
    }
