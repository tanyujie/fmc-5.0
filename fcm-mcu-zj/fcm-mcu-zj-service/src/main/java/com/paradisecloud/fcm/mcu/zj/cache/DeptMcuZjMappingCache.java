 ...
 * @version V1.0  
 */
public class DeptMcuZjMappingCache extends JavaCache<Long, BusiMcuZjDept>
{
    ...
    private static final DeptMcuZjMappingCache INSTANCE = new DeptMcuZjMappingCache();
     ...
     */
    private DeptMcuZjMappingCache()
    {
    }
    
    public static DeptMcuZjMappingCache getInstance()
    {
        return INSTANCE;
    }
    
    public BusiMcuZjDept getBindMcu(Long key)
    {
        BusiMcuZjDept busiMcuZjDept = super.get(key);
    ...
    public boolean isBindSameMcu(long... deptIds)
    {
        ...
        BusiMcuZjDept last = getBindMcu(deptIds[0]);
        for (int i = 1; i < deptIds.length; i++)
        {
            BusiMcuZjDept busiMcuZjDept = getBindMcu(deptIds[i]);
            if (!(McuZjType.convert(busiMcuZjDept.getMcuType()) == McuZjType.convert(last.getMcuType()) && busiMcuZjDept.getMcuId().longValue() == last.getMcuId().longValue()))
            {
                return false;
            }
        }
     ...
     * @return int
     */
    public int getBindDeptCount(McuZjType mcuXdType, long mcuXdId)
    {
        int c = 0;
        for (BusiMcuZjDept busiMcuZjDept : values())
        {
            if (McuZjType.convert(busiMcuZjDept.getMcuType()) == mcuXdType && busiMcuZjDept.getMcuId().longValue() == mcuXdId)
            {
                c++;
            }
        }
