...
public class McuZjSendConferenceInfoToTerminalTask extends DelayTask {

    private McuZjConferenceContext conferenceContext;
    private Long newPresenter;

    public McuZjSendConferenceInfoToTerminalTask(String id, long delayInMilliseconds, McuZjConferenceContext conferenceContext) {
        super("send_c_i_" + id, delayInMilliseconds);
        this.conferenceContext = conferenceContext;
    }

    public McuZjSendConferenceInfoToTerminalTask(String id, long delayInMilliseconds, McuZjConferenceContext conferenceContext, Long newPresenter) {
        ...
        if (conferenceContext != null) {
            ...
            if (newPresenter != null && newPresenter.longValue() != presenter.longValue()) {
                ...
                if (busiUserTerminalNewPresenter != null) {
                    AttendeeForMcuZj terminalAttendeePresenter = conferenceContext.getAttendeeByTerminalId(busiUserTerminalNewPresenter.getTerminalId());
                    if (terminalAttendeePresenter != null && terminalAttendeePresenter.getMeetingStatus() == AttendeeMeetingStatus.IN.getValue()) {
                        BusiTerminal busiTerminal = TerminalCache.getInstance().get(terminalAttendeePresenter.getTerminalId());
                        if (StringUtils.isNotEmpty(busiTerminal.getSn())) {
                            if (busiTerminal.getMqttOnlineStatus() == TerminalOnlineStatus.ONLINE.getValue()) {
