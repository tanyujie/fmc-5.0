 ...
 * 只用于MTR探测
 */
public abstract class SshRemoteServerOperateForOPS
{
	private static final Logger LOGGER = LoggerFactory.getLogger(SshRemoteServerOperateForOPS.class);
	private Session session;// JSCH session
	private boolean logined = false;// 是否登陆

	private static final SshRemoteServerOperateForOPS INSTANCE = new SshRemoteServerOperateForOPS()
	{

	};

	public static SshRemoteServerOperateForOPS getInstance()
	{
		return INSTANCE;
	}
