 ...
 * @version V1.0  
 */
public class SelfCallAttendeeDingProcessor extends AttendeeProcessor
{
     ...
     */
    public SelfCallAttendeeDingProcessor(SmcParitipantsStateRep.ContentDTO participant, DingConferenceContext conferenceContext)
    {
    ...
    public void process()
    {
        SelfCallAttendeeDing selfCallAttendee = new SelfCallAttendeeDing();
...

        {
            ...
            if(Strings.isNotBlank(remoteParty)){
                ...
                if (busiTerminal != null) {
                    ...
                    selfCallAttendee = new SelfCallAttendeeDing();
                    if (!ObjectUtils.isEmpty(participant.getGeneralParam().getName())) {
                        selfCallAttendee.setName(participant.getGeneralParam().getName());
                    } else {
                        selfCallAttendee.setName(participant.getGeneralParam().getName());
        ...
        if (addAttendee) {
            DingWebSocketMessagePusher.getInstance().upwardPushConferenceMessage(conferenceContext, WebsocketMessageType.ATTENDEE_ADD, selfCallAttendee);
            DingWebSocketMessagePusher.getInstance().upwardPushConferenceMessage(conferenceContext, WebsocketMessageType.MESSAGE_TIP, "【" + selfCallAttendee.getName() + "】入会");
            BeanFactory.getBean(IMqttService.class).sendJoinConferenceToPushTargetTerminal(conferenceContext, selfCallAttendee);

        }
        IBusiMcuDingHistoryConferenceService teleHistoryConferenceService = BeanFactory.getBean(IBusiMcuDingHistoryConferenceService.class);
            ...
            for (Object mcuAttendee : mcuAttendees) {
                ...
                if(Objects.equals(conferenceContext.getName(),name1)&&Objects.equals(name,UpbaseConferenceContext.getName())){
                    try {
                        ...
                        logger.error("MCU-Ding ChangeMasterAttendeeOperation error", e.getMessage());
                    }
                }
    ...
    private void setCommon(AttendeeDing attendee)
    {
