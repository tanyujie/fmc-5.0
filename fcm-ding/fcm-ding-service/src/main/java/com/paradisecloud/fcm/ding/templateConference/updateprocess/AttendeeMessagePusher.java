...
public class AttendeeMessagePusher
{
     ...
     */
    public void pushMeetingJoinedMessage(AttendeeDing attendee, SmcParitipantsStateRep.ContentDTO participant, DingConferenceContext conferenceContext)
    {
        if (attendee.containsUpdateField("meetingStatus"))
        {
            ...
            // 消息和参会者信息同步到主级会议
            DingWebSocketMessagePusher.getInstance().upwardPushConferenceMessage(conferenceContext, WebsocketMessageType.MESSAGE_TIP, messageTip);
        }
    }
     ...
     * @since 2021-02-04 17:40  void
     */
    public void pushMixingStatusMessage(AttendeeDing attendee, DingConferenceContext conferenceContext)
    {
        // 混音消息
     ...
     * @since 2021-02-04 17:40  void
     */
    public void pushOnlineMessage(AttendeeDing attendee, DingConferenceContext conferenceContext)
    {
        // 在线消息
        if (attendee.containsUpdateField("onlineStatus"))
        {
            ...
            {
                // 消息和参会者信息同步到主级会议
                DingWebSocketMessagePusher.getInstance().upwardPushConferenceMessage(conferenceContext, WebsocketMessageType.MESSAGE_ERROR, messageTip);
            }
            else
            {
                // 消息和参会者信息同步到主级会议
