...
import com.paradisecloud.fcm.ding.templateConference.updateprocess.OtherAttendeeUpdateProcessor;
import com.paradisecloud.fcm.ding.templateConference.updateprocess.RegisteredAttendeeUpdateProcessor;
...
public class AttendeeUtils {
    ...
    public static void updateByParticipant(DingConferenceContext conferenceContext, SmcParitipantsStateRep.ContentDTO participant, AttendeeDing a) {
        synchronized (a) {
            ...
            try {
                ...
                if (a instanceof TerminalAttendeeDing || a instanceof McuAttendeeDing) {
                ...
                if (a.isMeetingJoined() && !ObjectUtils.isEmpty(participant.getGeneralParam().getName())) {
                    ...
                    } else if (a instanceof TerminalAttendeeDing) {
                        terminalId = a.getTerminalId();
                    }
                    if (terminalId != null) {
                ...
                if (a.getUpdateMap().size() > 1) {
                    ...
                    DingWebSocketMessagePusher.getInstance().upwardPushConferenceMessage(conferenceContext, WebsocketMessageType.ATTENDEE_UPDATE, new HashMap<>(a.getUpdateMap()));
                    if (a instanceof McuAttendeeDing) {
                        if (a.getUpdateMap().containsKey("meetingStatus")) {
                            DingWebSocketMessagePusher.getInstance().upwardPushConferenceMessage(conferenceContext, WebsocketMessageType.ATTENDEE_UPDATE, a);
                        }
                    }
                }
    ...
    public static AttendeeDing matchAttendee(DingConferenceContext conferenceContext, SmcParitipantsStateRep.ContentDTO participant) {

        if (!ObjectUtils.isEmpty(participant.getAttendeeId())) {
            AttendeeDing a = conferenceContext.getAttendeeById(participant.getAttendeeId());
        ...
        if (remoteParty != null) {
            ...
            Map<String, AttendeeDing> uuidAttendeeMap = conferenceContext.getUuidAttendeeMapByUri(remoteParty);
            if (uuidAttendeeMap != null) {
                for (Map.Entry<String, AttendeeDing> stringAttendeeTeleEntry : uuidAttendeeMap.entrySet()) {
                    AttendeeDing value = stringAttendeeTeleEntry.getValue();
        ...
        Map<String, AttendeeDing> uuidAttendeeMap = null;
        ...
        if (!ObjectUtils.isEmpty(participant.getAttendeeId())) {
            if (uuidAttendeeMap != null) {
                AttendeeDing a = uuidAttendeeMap.get(participant.getAttendeeId());
        ...
        synchronized (participant) {
            ...
            AttendeeDing a = matchAttendee(participant, uuidAttendeeMap);
            if (a == null) {
                if (uuidAttendeeMap != null) {
    ...
    public static AttendeeDing matchAttendee(SmcParitipantsStateRep.ContentDTO participant, Map<String, AttendeeDing> uuidAttendeeMap) {
        if (uuidAttendeeMap == null) {
            return null;
        }
        for (Iterator<AttendeeDing> iterator = uuidAttendeeMap.values().iterator(); iterator.hasNext(); ) {
            AttendeeDing a = iterator.next();
            synchronized (a) {
    ...
    public static List<AttendeeDing> getAll(DingConferenceContext conferenceContext) {
        List<AttendeeDing> list = new ArrayList<>();
        if (conferenceContext == null) {
            return list;
        }
        List<AttendeeDing> attendees = conferenceContext.getAttendees();
        if (attendees != null && attendees.size() > 0) {
            list.addAll(attendees);
        }
        AttendeeDing masterAttendee = conferenceContext.getMasterAttendee();
        if (masterAttendee != null) {
            list.add(masterAttendee);
        }
        List<AttendeeDing> masterAttendees = conferenceContext.getMasterAttendees();
        if (masterAttendees != null && masterAttendees.size() > 0) {
            list.addAll(masterAttendees);
        }

        Map<Long, List<AttendeeDing>> cascadeAttendeesMap = conferenceContext.getCascadeAttendeesMap();
        if (cascadeAttendeesMap != null) {
            Collection<List<AttendeeDing>> values = cascadeAttendeesMap.values();
            for (List<AttendeeDing> value : values) {
                list.addAll(value);
            }
