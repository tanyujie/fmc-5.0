...
import com.paradisecloud.fcm.ding.service2.interfaces.IBusiRecordsForMcuDingService;
 ...
 */
@Service
public class BusiRecordsServiceForMcuDingImpl implements IBusiRecordsForMcuDingService {
    ...
    @Resource
    private IBusiDingConferenceService iBusiDingConferenceService;
    ...
    public int updateBusiRecords(boolean recording, String contextKey) {
        ...
        boolean success = iBusiDingConferenceService.updateCallRecordStatus(contextKey, recording);
    ...
    public int getRecordSpaceStatus() {
        ...
        Collection<DingConferenceContext> values = DingConferenceContextCache.getInstance().values();
        int i = 1;
        if (values != null && values.size() > 0) {
            for (DingConferenceContext value : values) {
                if (value.isRecorded()) {
                    i++;
                }
            }
     ...
     * @param path
     */
    private void readRecordFileName(String conferenceNumber, DingConferenceContext conferenceContext, BusiRecords busiRecords, String path) {
