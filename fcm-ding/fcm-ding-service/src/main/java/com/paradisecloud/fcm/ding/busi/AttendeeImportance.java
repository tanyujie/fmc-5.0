...
public enum AttendeeImportance {
    ...
    COMMON(null, null, "分会场") {
        @Override
        public void processAttendeeWebsocketMessage(AttendeeDing attendee) {
    ...
    MASTER(7500000, 7500000, "主会场") {
        ...
        public void processAttendeeWebsocketMessage(AttendeeDing attendee) {
            ...
            attendee.setBroadcastStatus(BroadcastStatus.NO.getValue());
            attendee.setRoundRobinStatus(AttendeeRoundRobinStatus.NO.getValue());
            DingConferenceContext mainConferenceContext = DingConferenceContextCache.getInstance().get(attendee.getContextKey());
            if (mainConferenceContext != null && mainConferenceContext.getMasterAttendee() != attendee) {
                AttendeeDing oldMasterAttendee = mainConferenceContext.getMasterAttendee();
                ...
                DingWebSocketMessagePusher.getInstance().pushConferenceMessageToAll(mainConferenceContext, WebsocketMessageType.MASTER_CHANGED, data);

                StringBuilder messageTip = new StringBuilder();
                messageTip.append("主会场已切换至【").append(attendee.getName()).append("】");
                DingWebSocketMessagePusher.getInstance().pushConferenceMessageToAll(mainConferenceContext, WebsocketMessageType.MESSAGE_TIP, messageTip);
        ...
        public void processAttendeeWebsocketMessage(AttendeeDing attendee) {
            ...
            if (attendee.getUpdateMap().size() > 1) {
                ...
                if (mainConferenceContext != null) {
                    ...
                    DingWebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(mainConferenceContext, WebsocketMessageType.MESSAGE_TIP, messageTip);
                    DingWebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(mainConferenceContext, WebsocketMessageType.ATTENDEE_UPDATE, attendee.getUpdateMap());
                }
            }
        }
    },
     ...
     */
    public abstract void processAttendeeWebsocketMessage(AttendeeDing attendee);
