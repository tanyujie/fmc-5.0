 ...
 */
public enum DingWebsocketMessageType {
    /**
     * 会监号码
     */
     ...
     */
    private String name;
    private static final Map<String, DingWebsocketMessageType> MAP = new HashMap<>();
    static
    {
        for (DingWebsocketMessageType recordType : values())
        {
            MAP.put(recordType.value, recordType);
        }
    }

    DingWebsocketMessageType(String value, String name)
    {
        this.value = value;
        this.name = name;
    }
    ...
    public DingWebsocketMessage create(Object data)
    {
        return new DingWebsocketMessage(value, data);
    }


    public DingWebsocketMessage create(Object data, String destination)
    {
        return new DingWebsocketMessage(value, data, destination);
    }

    public static DingWebsocketMessageType convert(String value)
    {
        DingWebsocketMessageType t = MAP.get(value);
        if (t == null)
        {
            throw new SystemException("非法的" + DingWebsocketMessageType.class.getSimpleName() + "枚举值：" + value);
        }
        return t;
    }
}
