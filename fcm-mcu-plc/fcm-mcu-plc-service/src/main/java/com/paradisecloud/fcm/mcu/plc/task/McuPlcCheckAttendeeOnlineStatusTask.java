...
public class McuPlcCheckAttendeeOnlineStatusTask extends DelayTask {

    public McuPlcCheckAttendeeOnlineStatusTask(String id, long delayInMilliseconds) {
    ...
    @Override
    public void run() {
        Collection<McuPlcConferenceContext> conferenceContexts = McuPlcConferenceContextCache.getInstance().values();
        if (conferenceContexts != null) {
            for (McuPlcConferenceContext conferenceContext : conferenceContexts) {
                updateConference(conferenceContext);
            }
        }
    }

    private void updateConference(McuPlcConferenceContext conferenceContext) {
        try {
            if (!conferenceContext.isEnd()) {
                ...
                for (TerminalAttendeeForMcuPlc terminalAttendee : terminalAttendeeMap.values()) {
                    ...
                    synchronized (terminalAttendee) {
                        ...
                        if (terminalAttendee.containsUpdateField("onlineStatus")) {
                            ...
                            McuPlcWebSocketMessagePusher.getInstance().upwardPushConferenceMessage(conferenceContext, WebsocketMessageType.ATTENDEE_UPDATE, updateMap);

                        }
                    }
                }
                List<McuAttendeeForMcuPlc> mcuAttendees = conferenceContext.getMcuAttendees();
                for (McuAttendeeForMcuPlc mcuAttendee : mcuAttendees) {
                    ...
                    synchronized (mcuAttendee) {
                        ...
                        if (mcuAttendee.containsUpdateField("onlineStatus")) {
                            ...
                            // 消息和参会者信息同步到主级会议
                            McuPlcWebSocketMessagePusher.getInstance().upwardPushConferenceMessage(conferenceContext, WebsocketMessageType.MESSAGE_TIP, messageTip);
                            McuPlcWebSocketMessagePusher.getInstance().upwardPushConferenceMessage(conferenceContext, WebsocketMessageType.ATTENDEE_UPDATE, mcuAttendee.getUpdateMap());
