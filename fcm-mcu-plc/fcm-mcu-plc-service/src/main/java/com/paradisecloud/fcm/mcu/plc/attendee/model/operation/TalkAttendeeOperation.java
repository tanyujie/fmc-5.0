...
public class TalkAttendeeOperation extends AttendeeOperation {
    ...
    private AttendeeForMcuPlc talkAttendee;
     ...
     */
    public TalkAttendeeOperation(McuPlcConferenceContext conferenceContext, AttendeeForMcuPlc talkAttendee) {
    ...
    private void operateScreen() {
        ...
        if (currentTimeMillis - getLastUpdateTime() >= 10000) {
            ...
            if (masterAttendee != null && StringUtils.isNotEmpty(masterAttendee.getParticipantUuid())) {
                ...
                cellInfo.setForceId(talkUuid);
                cellInfo.setSourceId(talkUuid);
            ...
            }
            {
                ...
                ccUpdateMrMosicConfigRequestForGuest.setLayout(McuPlcLayoutTemplates.SCREEN_2.getCode());
                ...
                CellInfo cellInfoForGuest2 = new CellInfo();
                cellInfoForGuest2.setId("2");
                cellInfoForGuest2.setForceStatus("forced");
                cellInfoForGuest2.setForceId(talkUuid);
                cellInfoForGuest2.setSourceId(talkUuid);
                cellInfoListForGuest.add(cellInfoForGuest2);
            ...
            }
            {
                ...
                CcUpdatePersonalMosicConfigRequest ccUpdatePersonalMosicConfigRequest = new CcUpdatePersonalMosicConfigRequest();
                ccUpdatePersonalMosicConfigRequest.setId(conferenceContext.getConfId());
                ccUpdatePersonalMosicConfigRequest.setParty_id(talkUuid);
                ccUpdatePersonalMosicConfigRequest.setLayout_type("personal");
                ccUpdatePersonalMosicConfigRequest.setLayout(McuPlcLayoutTemplates.SCREEN_1.getCode());
                List<CellInfo> cellInfoList = new ArrayList<>();
                CellInfo cellInfo = new CellInfo();
                ...
                if (lastUpdateTime == 0) {
                    for (AttendeeForMcuPlc attendeeForMcuPlc : autoPollingAttends) {
                        if (attendeeForMcuPlc != null && StringUtils.isNotEmpty(attendeeForMcuPlc.getParticipantUuid())) {
                            if (talkUuid.equals(attendeeForMcuPlc.getParticipantUuid())) {
                                continue;
                            }
            ...
            if (success) {
                ...
                // 打开举手混音
                {
                    CcUpdateTerminalAudioAndVideoRequest ccUpdateTerminalAudioAndVideoRequest = new CcUpdateTerminalAudioAndVideoRequest();
                    ccUpdateTerminalAudioAndVideoRequest.setId(conferenceContext.getConfId());
                    ccUpdateTerminalAudioAndVideoRequest.setParty_id(talkUuid);
                    ccUpdateTerminalAudioAndVideoRequest.setAudio_mute(false);
                    if (talkAttendee.getVideoStatus() == AttendeeVideoStatus.YES.getValue()) {
                        ccUpdateTerminalAudioAndVideoRequest.setVideo_mute(false);
                    } else {
                        ccUpdateTerminalAudioAndVideoRequest.setVideo_mute(true);
                    }
                    ...
                    if (ccUpdateTerminalAudioAndVideoResponse != null && CommonResponse.STATUS_OK.equals(ccUpdateTerminalAudioAndVideoResponse.getStatus())) {
                        ...
                        talkAttendee.setMixingStatus(AttendeeMixingStatus.YES.getValue());
                        McuPlcWebSocketMessagePusher.getInstance().upwardPushConferenceMessage(conferenceContext, WebsocketMessageType.ATTENDEE_UPDATE, talkAttendee.getUpdateMap());
                    }
                }
                if (success) {
                    // 发送提示信息
                    McuPlcWebSocketMessagePusher.getInstance().pushSpecificConferenceMessage(conferenceContext, WebsocketMessageType.MESSAGE_TIP, AttendeeMixingStatus.YES.getName());
                    StringBuilder messageTip = new StringBuilder();
