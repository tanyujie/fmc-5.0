...
public class UpdateAttendeeMediaInfoTask extends DelayTask {
    ...
    public UpdateAttendeeMediaInfoTask(String id, long delayInMilliseconds, McuPlcConferenceContext conferenceContext, AttendeeForMcuPlc attendee, BusiHistoryParticipant busiHistoryParticipant) {
    ...
    public void run() {
        ...
        if (conferenceContext != null && attendee != null) {
            ...
            for (int i = 0; i < 3; i++) {
                try {
                    jsonObject = attendeeForMcuPlcService.detail(conferenceContext, attendee);
                    if (jsonObject != null) {
